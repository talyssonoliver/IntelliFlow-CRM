...
| `/tickets/sla-policies` | SLA Policies & Settings | 5-card module-settings bento (PG-185): SLA policies (REUSES ticketConfig.slaPolicy), duplicate detection, required fields, tags, automation (21 toggles incl. default SLA, auto-close, notification triggers, AI defaults off). Route lives under `tickets/(list)/sla-policies/` so the Ticket settings sidebar applies. |
...
> **PG-186**: `document-settings`, `document-types`, and `storage-policies`
> pages moved into `(list)/` route group so the `DocumentSettingsSidebarNav`
> layout applies. URLs unchanged.

| Route                          | Page              | Description                                        | Filesystem                                                        |
| ------------------------------ | ----------------- | -------------------------------------------------- | ----------------------------------------------------------------- |
| `/documents`                   | Documents List    | Document repository                                | `app/documents/(list)/page.tsx`                                   |
| `/documents/new`               | Upload Document   | Document upload form                               | `app/documents/new/page.tsx`                                      |
| `/documents/[id]`              | Document Detail   | Document preview and metadata                      | `app/documents/[id]/page.tsx`                                     |
| `/documents/document-types`    | Document Types    | Configure document type labels and metadata fields | `app/documents/(list)/document-types/page.tsx` (moved — PG-186)   |
| `/documents/storage-policies`  | Storage Policies  | Retention rules, archival thresholds, quota limits | `app/documents/(list)/storage-policies/page.tsx` (moved — PG-186) |
| `/documents/document-settings` | Document Settings | Full module settings: types, antivirus, retention  | `app/documents/(list)/document-settings/page.tsx` (PG-186)        |

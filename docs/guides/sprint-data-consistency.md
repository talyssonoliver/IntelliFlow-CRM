...
- Edit `task-registry.json` manually (it is derived + gitignored; regenerate
  with `pnpm regenerate:derived`)
...
4. Updates task-registry.json, \_summary.json, phase summaries (these are all
   derived files; the canonical regenerator is `pnpm regenerate:derived`)

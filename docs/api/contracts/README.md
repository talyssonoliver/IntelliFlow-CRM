...
> **Source of Truth**: `api-contracts.yaml` **Validator**:
> `packages/validators/__tests__/api-contract-consistency.spec.ts` **Last
> Updated**: 2026-01-05 **Tokens**: ~800 (optimized for LLM context)
...
_No known violations_ - All issues have been resolved.
...
**VIOLATION-001 Resolution**: Changed `lead.stats` to use `tenantProcedure` with
`createTenantWhereClause()`. Dashboard and Leads page now show consistent data.

**VIOLATION-002 Resolution**: Changed `documents.router.ts` to use
`ctx.user?.tenantId` instead of `userId`. Fixed multi-tenant isolation for
document storage.

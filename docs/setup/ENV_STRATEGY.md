...
4. **No duplicate .env files** in packages or apps (except for Next.js
   NEXT*PUBLIC*\*)
...
❌ Don't create `.env` files in `packages/db/`, `apps/api/`, etc. ❌ Don't
...
✅ Keep `apps/web/.env.local` for NEXT*PUBLIC*\* vars only

## Special Cases

### Next.js (apps/web)

Next.js auto-loads `.env.local` for client-side variables. Only put
`NEXT_PUBLIC_*` variables in `apps/web/.env.local`. Server-side vars come from
root `.env` via dotenv-cli.

### Prisma (packages/db)

Prisma reads DATABASE_URL from the environment. The root `.env` is loaded by
dotenv-cli before any db: command.

### Tests

Test commands use `dotenv -e .env` to load environment. For test-specific
overrides, use `.env.test`.

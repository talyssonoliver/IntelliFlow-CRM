...
Ship real-time analytics dashboards that read live Supabase data, maintain
freshness SLAs, and meet performance and data-quality targets.
...
- Charts backed by Supabase realtime channels; fallback polling only for
  outages.
- KPI definitions versioned; data-quality assertions (non-null, non-placeholder)
  in CI.
...
- Observability: traces/spans for dashboard queries; perf video + latency JSON
  artifacts.
...
- Risk: Realtime channel throttling → Mitigate with backoff and fallback
  polling.
- Risk: Data-quality regressions → Mitigate with CI assertions and seeded
  fixtures.

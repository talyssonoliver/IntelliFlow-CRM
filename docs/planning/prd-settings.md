...
IntelliFlow CRM requires a centralized settings interface for users to manage
their account preferences, security settings, AI configuration, and team
management.

### Problem Description

Without a unified settings UI, configuration is scattered across multiple entry
points, leading to a fragmented user experience.

## User Stories

### User Story 1

**As a** user **I want to** access all settings from one page **So that** I can
manage my preferences efficiently.

### User Story 2 (IFC-191)

**As a** user **I want to** set my preferred timezone in account settings **So
that** the home page greeting and time-sensitive features display correctly for
my location.
...
- AC-3: `getGreeting()` in `home.router.ts` uses user's timezone (not server
  time)
- AC-4: Fallback chain: user timezone → UTC (tenant-level timezone is a future
  enhancement; Tenant model has no timezone field)
- AC-5: TimezoneSelector component on `/settings/account` page allows timezone
  selection

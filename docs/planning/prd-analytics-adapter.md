...
The analytics router (`apps/api/src/modules/analytics/analytics.router.ts`) provided dashboard endpoints but relied on `AnalyticsService` — a service that directly consumed `PrismaClient`, violating hexagonal architecture. This created:

1. **Untestable business logic** — service methods were coupled to Prisma, requiring database for any test
2. **No export pipeline** — metrics could only be viewed in the dashboard, not exported
3. **Performance issues** — serial N+1 queries in `getGrowthTrend` (loop per month)
4. **Broken normalization** — growth values were clamped to 0-100 range, making raw data inaccessible

## Requirements

### Functional
- **FR-001**: Create `AnalyticsAggregationService` in application layer accepting `AnalyticsRepository` port
- **FR-002**: Wire `PrismaAnalyticsRepository` (pre-existing) through DI container

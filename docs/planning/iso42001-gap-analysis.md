...
| Sentiment Analysis       | LiteLLM proxy (Mistral for EU PII, Groq for non-PII) | Analyze customer interactions   | Low        | Automated                  |
...
| Privacy controls                  | Basic RLS + field-level AES-256-GCM encryption for AI prompts (`ChainVersion.prompt`, `ChainVersion.systemPrompt`) and credentials (`WebhookEndpoint.secret`) — delivered sprint-17 (M4) via bespoke `fieldEncryptionExtension` (`packages/db/src/field-encryption.ts`). Storage format `v1:<iv>:<authTag>:<ciphertext>`. Maps to ISO 27001 A.8.24 + GDPR Art. 32. | Full privacy-by-design review + `CallRecord` PII fields (DBA-016: `fromNumber`, `toNumber`, `contactName`, `userName`) in follow-on sprint. | **Delivered** — prompt encryption active sprint-17 via bespoke Prisma 7 `$extends` extension (replaces incompatible `prisma-field-encryption@1.x`); `CallRecord` PII deferred (separate UX review required for E.164 format preservation). | High → Medium |
...
1. **Establish AI Governance Structure**
   ...
   - **Deliverable**:
     `docs/compliance-and-governance/governance/ai-governance-policy.md`
...
3. **Create AI Risk Register**
   ...
   - **Deliverable**:
     `docs/compliance-and-governance/governance/ai-risk-register.md`
...
10. **Establish Vendor Management Process**
    ...
    - **Deliverable**:
      `docs/compliance-and-governance/governance/ai-vendor-assessments/`

...
Golden paths are the recommended, supported ways to accomplish common
development tasks in the IntelliFlow monorepo. Each path has a single entrypoint
command, documented prerequisites, and a clear owner.
...
**Workflow**: Create feature branch, implement in `apps/web/`, write tests, open
PR, CI validates, merge.
...
**Workflow**: Define Zod schema in `packages/validators/`, create domain model,
build tRPC router, write tests, open PR.
...
**Workflow**: Edit `schema.prisma`, run `db:migrate:create`, verify migration
SQL, apply with `db:migrate`, regenerate client.
...
- All metric values must include provenance (source, collection method,
  timestamp)

...
**Symptoms**: Events appearing late, `intelliflow_outbox_batch_size`
consistently high
...
- **Events Worker**: Scale carefully - outbox polling should be single-instance
  per partition
- **Ingestion Worker**: Scales well horizontally, no shared state
- **Notifications Worker**: Scale based on channel; email/SMS have provider rate
  limits
...
See `infra/monitoring/alerts/intelliflow-alerts.yaml` for Prometheus alert
rules:
...
- [Notifications Runbook](./runbooks/notifications.md) - Notification delivery
  specifics
- [Event Contracts](../events/contracts-v1.yaml) - Event schema definitions
- [Architecture: Hex Boundaries](../architecture/hex-boundaries.md) - System
  design

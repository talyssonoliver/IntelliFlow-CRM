...
Document Settings Chain (PG-186):
  document-settings.router.ts ──► document-automation.ts ──► documents.router.ts (create/update/delete)
  document-settings.router.ts is registered in apps/api/src/router.ts as documentSettings: documentSettingsRouter
  document-automation.ts depends on @intelliflow/db (DocumentAutomationSetting type)
  Category-1 helpers (normalizeFilename, assertNotDeleteGuarded, notifyDocumentReassignment) wired into documents.router.ts
  Category-2 helpers (autoDetectDuplicates, autoVersionOnCollision, AI features) deferred to IFC-310/IFC-312
```
...
DOC-007 (Gap Assessment) ─── COMPLETE
  └── DOC-008 (VPAT + CI Enforcement) ─── COMPLETE
        Artifacts:
          docs/compliance-and-governance/compliance/vpat-2.5.md
          ...
          tests/a11y/setup.ts
          docs/architecture/adr/ADR-038-accessibility-architecture.md
        Config changes:
...
apps/web/src/lib/leads/lead-enrichment.ts        (pure module: derive website/company from email domain)
  └─ apps/web/src/app/leads/(list)/new/page.tsx  (New Lead 3-step wizard; email onBlur → enrichFromEmail)
      └─ api.lead.create (tRPC)                   (existing; ownerId = creator → "assignment automated")
```

Status: ✅ Complete (PG-060 sprint 18). Pure module `lead-enrichment.ts` meets
strict coverage thresholds (97.56 / 94.28 / 100 / 100 Istanbul). Enrichment is
non-destructive (only fills blank website/company) and adds zero submission
latency (no network/LLM). "assignment automated" satisfied by the existing
`ownerId = creator` behavior in `lead.router.ts`.
...
```
PG-184 (Deal Settings) runtime chain:
  /deals/deal-settings (page.tsx)
    ├── DealSettingsContent (orchestrator, client)
    │   ├── DealPipelineCard → trpc.pipelineConfig.* (IFC-063 reuse)
    │   └── Deal{WinLoss,Scoring,DuplicateDetection,RequiredFields,Tags,Automation}Card
    │       → trpc.dealSettings.*  →  deal-settings.router.ts
    └── opportunity.router.ts (create/update/delete)
        └── deal-automation.ts  →  DealAutomationSetting (Prisma)

Category-1 toggles are consumed at runtime by deal-automation.ts; category-2
(AI) toggles are follow-ups registered against IFC-312.
```

```
PG-185 (Ticket Settings) runtime chain:
  /tickets/(list)/sla-policies (page.tsx + Suspense shell)
    ├── TicketSettingsContent (orchestrator, client)
    │   ├── TicketSLAPoliciesCard → trpc.ticketConfig.slaPolicy.* (PG-173 reuse via ticketSettings.slaPolicies re-export)
    │   └── Ticket{DuplicateDetection,RequiredFields,Tags,Automation}Card
    │       → trpc.ticketSettings.*  →  ticket-settings.router.ts
    └── ticket.router.ts (create/update/delete/escalate)
        └── ticket-automation.ts  →  TicketAutomationSetting (Prisma)
            └── defaultSlaPolicyId FK → SLAPolicy (ON DELETE SET NULL)

Category-1 toggles (data hygiene, delete guard, notification gates, default
SLA resolver) are consumed at runtime by ticket-automation.ts; category-2
toggles: 4 auto-close knobs are follow-ups registered against IFC-310
(rule evaluator cron); 6 AI toggles are follow-ups on IFC-312.

SLA watcher (separate worker, IFC-067) consults
shouldWriteSlaBreachNotification / shouldWriteSlaWarningNotification from
ticket-automation.ts before emitting SLANotification rows.
```
...
- `AccountDuplicateDetectionService`
  (`apps/api/src/modules/account/account-duplicate-detection.service.ts`)
- `duplicate-rule-evaluator` (`apps/api/src/shared/duplicate-rule-evaluator.ts`)
  — pure function
- `ContactEmbedWorker` (`apps/ai-worker/src/workers/contact-embed-worker.ts`)
- `ContactMergedEvent`
  (`packages/application/src/services/events/ContactMergedEvent.ts`)
...
- `ContactService.mergeContacts → EventBus.publish(ContactMergedEvent)`
  (transactional outbox via ADR-011)
...
- `EnrichmentProvider` port (`apps/ai-worker/src/shared/enrichment-adapter.ts`)
  with `LiteLLMEnrichmentAdapter` + `MockEnrichmentAdapter` implementations.

New chain modules (10):

- `contact-enrichment.chain.ts` — consumes `EnrichmentProvider.enrichContact`;
  fills only empty Contact fields.
- `account-enrichment.chain.ts` — consumes `EnrichmentProvider.enrichAccount`;
  validates industry against tenant vocabulary.
- `contact-tag-suggestion.chain.ts` — sync suggest via `withTimeout(5s)`; caps
  at 5, filters confidence < 0.3.
- `account-tag-suggestion.chain.ts` — same shape for accounts.
- `contact-insight.chain.ts` — upserts `ContactAIInsight` with provenance
  fields.
- `account-insight.chain.ts` — upserts `AccountAIInsight` (new model).
- `contact-reply-draft.chain.ts` — returns payload only; job handler writes
  `ContactReplyDraft` with hard-coded `status='DRAFT'` (ADR-037 compliance).
- `account-industry-inference.chain.ts` — `z.enum(vocabulary.keys)` restriction
  at invoke time prevents off-taxonomy values.
- `account-scoring.chain.ts` — writes
  `Account.score`/`scoreProvenance`/`scoredAt`/`scoreModelVersion`.

New BullMQ queues + job handlers (5):

- `AI_ENRICHMENT` → `enrichment.job.ts` (discriminates contact|account).
- `AI_ENTITY_INSIGHT` → `entity-insight.job.ts` (distinct from lead-only
  `AI_INSIGHTS`).
- `AI_REPLY_DRAFT` → `reply-draft.job.ts` (writes ContactReplyDraft with DRAFT
  status only).
- `AI_ACCOUNT_SCORING` → `account-scoring.job.ts`.
- `AI_TAG_SUGGESTION` → `tag-suggestion.job.ts` (synchronous via
  `waitUntilFinished(5s)`).

All 5 job handlers re-check the relevant `aiXxx` toggle at job start (race
safety against flag flips between enqueue and execute).

New Prisma models + scalars:

- `AccountAIInsight` (new model) — one-per-account insight row with provenance.
- `ContactReplyDraft` (new model) — drafts always inserted with
  `status='DRAFT'`.
- `Account.score/scoreProvenance/scoredAt/scoreModelVersion` (4 nullable
  scalars) + `@@index([score])`.
- `Account.industryInferredAt/industryModelVersion` (2 nullable scalars).
- `ContactAIInsight.modelVersion/generatedAt/source` (3 nullable provenance
  scalars).
...
- `AccountScoreBadge` (accounts) — score badge with tier coloring + factors
  tooltip.

...
This document describes how traces, logs, and metrics are correlated in
IntelliFlow CRM's observability stack. Proper correlation enables:

- **Root Cause Analysis**: Jump from metric spike → trace → logs in <2 minutes
  (MTTD target)
...
**How it works**: Every span has a `trace_id` that groups related operations.
Logs automatically include this ID via the Pino mixin.
...
**How it works**: Each operation within a trace has a unique `span_id`. Logs
include the active span ID.
...
**How it works**: A `correlation_id` is generated at the API gateway and
...
**How it works**: All telemetry includes resource attributes identifying the
service.
...
**How it works**: Propagate business entities (leadId, userId, etc.) through the
stack.
...
1. **Find metric spike**:
   - Grafana → Prometheus
   - Query:
     `histogram_quantile(0.95, rate(intelliflow_api_request_duration_bucket[5m]))`
...
- Grafana Correlation:
  https://grafana.com/docs/grafana/latest/explore/correlations/

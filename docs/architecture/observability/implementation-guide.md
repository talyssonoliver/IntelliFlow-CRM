...
This guide provides step-by-step instructions for adding observability to
IntelliFlow CRM services using OpenTelemetry (OTel). The implementation provides
unified traces, metrics, and logs with automatic correlation.
...
The `@intelliflow/observability` package is already available in the monorepo.

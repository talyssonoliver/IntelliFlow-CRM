...
1. **Numbering**: Use sequential `ADR-NNN` numbering based on chronological
   order; never reuse a retired number.
2. **Status**: Start as `Proposed`; promote to `Accepted` after review, or
   `Rejected` / `Deprecated` / `Superseded by ADR-YYY`.
3. **Keep it concise**: ADRs should be scannable — aim for 1–2 pages on the core
   decision (deep implementation notes can extend).
4. **Focus on "why"**: Document the reasoning and context, not just the
   decision.
5. **Update status**: Mark `Deprecated` or `Superseded` when replaced by newer
   decisions; reference the replacement ADR.
6. **Link related ADRs**: Create a web of architectural knowledge via `Links`
   section.
...
2. Submit as PR with the template filled out
...
6. Merge PR to make the decision official

---

**Related:**

- [Decision Workflow](../../architecture/decision-workflow.md) — end-to-end
  process for making decisions
- [ARP Template](../../architecture/arp/000-template.md) — for proposing

...
> **ℹ️ Provider examples updated by
> [ADR-048](./ADR-048-hybrid-ai-inference.md).** Eval suite must be re-run with
> each LiteLLM-routed provider and results tagged with `provider` + `tier`.

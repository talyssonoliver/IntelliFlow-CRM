...
- Postman collection: `packages/test-fixtures/postman-collection.json`
...
- `LITELLM_BASE_URL=http://localhost:4000/v1` (primary path — LiteLLM proxy)
  Start with: `docker compose -f infra/docker/docker-compose.litellm.yml up -d`
- `OPENAI_API_KEY` for production (used internally by the LiteLLM proxy for the
  `premium` tier)
- Ollama as an offline alternative (`ollama serve`, model

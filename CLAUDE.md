...
1. `pnpm regenerate:derived` — content-hash cached; produces `Sprint_plan_A.csv`
   … `Sprint_plan_J.csv` locally (gitignored). Cache hits on warm runs are <100
   ms.
2. `GET http://localhost:3002/api/sprint-plan?range=A` (project-tracker running)
   — serves the regenerated split lazily and caches per request.

The range letters cover roughly 60–70 rows each across sprints 0–29; A is the
earliest, J is the most recent. `task-registry.json` and `dependency-graph.json`
are also derived now — produce them with

...
**CRITICAL**: There is NO WARN verdict. Security findings at ANY severity must
be resolved. MEDIUM findings that were previously WARN'd silently became
accepted tech debt. Fix them or document the risk in an ADR.
...
Unavailable tools do NOT produce a PASS — they produce no result. The verdict is
based on gates that DID run. If zero security gates could run, verdict is
NEEDS_HUMAN.
...
- Run available gates even if some tools are missing (document unavailable
  tools)

...
$csvPath = "artifacts/recovery/snapshot-c46fc8b/Sprint_plan.csv"
$flowsDir = "apps/project-tracker/docs/metrics/_global/flows"
$outPath = "artifacts/recovery/snapshot-c46fc8b/feature-matrix-recovered-c46.md"
...
function Shorten([string]$text, [int]$maxLen = 72) {
  $t = Clean-Cell $text
...
function Format-IdList([string[]]$ids, [int]$max = 4) {
...
function Format-RefList([string[]]$refs, [int]$max = 2) {
  if (-not $refs -or $refs.Count -eq 0) { return '-' }
  $uniq = @($refs | Where-Object { $_ -and $_.Trim() -ne '' } | ForEach-Object { Clean-Cell $_ } | Sort-Object -Unique)
  if ($uniq.Count -eq 0) { return '-' }
  if ($uniq.Count -le $max) { return ($uniq -join ', ') }
  $head = $uniq[0..($max - 1)] -join ', '
  return "$head +$($uniq.Count - $max)"
...
function Get-Group([string]$section, [string]$text) {
  ...
  if ($s -match 'report|analytics|dashboard|metrics') { return 'Analytics' }
...
function Get-Priority([string]$normalizedStatus, [string]$group, [string]$targetSprint) {
  ...
  $core = @('Lead', 'Contact', 'Account', 'Deal', 'Ticket', 'Auth', 'Billing', 'Notification', 'AI', 'Marketing', 'Automation', 'Case', 'Document', 'Dashboard')
  if ($group -in $core) {
    $n = 0
...
function Get-RequiredLayers([string]$group, [string]$section, [string]$primaryTaskId) {
  ...
  switch ($group) {
    'Marketing' { return $required }
    'Developer' { return $required }
    'Legal' { return $required }
    'Auth' {
  ...
  if ($group -in @('Governance', 'Platform')) {
...
function Get-LayerCell($infos, [string]$layerName, [bool]$required) {
  ...
  $ids = $hits | Select-Object -ExpandProperty TaskId
  return "$state ($(Format-IdList $ids 2))"
}
...
foreach ($r in $pgRows) {
  foreach ($id in (Get-DependencyIds $r.Dependencies)) { if ($id -match '^IFC-') { [void]$ifcSet.Add($id) } }
}
...
foreach ($f in $baseFeatures) {
  ...
  $required = Get-RequiredLayers $f.Group $f.PrimarySection $f.PrimaryTaskId
  ...
  $specRefs = New-Object System.Collections.Generic.List[string]
  if ($primaryInfo) {
    foreach ($s in $primaryInfo.Specs) { $specRefs.Add((Shorten $s 42)) }
    foreach ($a in $primaryInfo.Artifacts) {
      $aClean = Clean-Cell $a
      if ($aClean -match 'docs/specs/|docs/planning/prd|docs/planning/adr') { $specRefs.Add((Shorten $aClean 42)) }
    }
  }
  $reqRef = "$($f.PrimaryTaskId)"
  $specUniq = @($specRefs | Sort-Object -Unique)
  if ($specUniq.Count -gt 0) { $reqRef = "$($f.PrimaryTaskId); " + (Format-IdList $specUniq 2) }
  ...
  $flowRef = Format-RefList @($flowRefs) 2
  $prdRef = Format-RefList @($prdRefs) 2
  $adrRef = Format-RefList @($adrRefs) 2

  $kpiIds = @($infos | Where-Object { $_.KPI -and $_.KPI.Trim() -ne '' } | Select-Object -ExpandProperty TaskId | Sort-Object -Unique)
  $kpiRef = if ($kpiIds.Count -gt 0) { Format-IdList $kpiIds 3 } else { '-' }

  $valRefs = @($infos | Where-Object { $_.Validation -and $_.Validation.Trim() -ne '' } | ForEach-Object { "$($_.TaskId): $(Shorten $_.Validation 48)" } | Sort-Object -Unique)
  $validationRef = if ($valRefs.Count -eq 0) { '-' } elseif ($valRefs.Count -le 2) { ($valRefs -join '; ') } else { "$($valRefs[0]); $($valRefs[1]) +$($valRefs.Count - 2)" }

  $evidenceRef = '-'
  if ($primaryInfo) {
    $ev = @($primaryInfo.Evidence | Sort-Object -Unique)
    if ($ev.Count -gt 0) { $evidenceRef = Format-IdList (@($ev | ForEach-Object { Shorten $_ 44 })) 2 }
  }
  ...
  $matrix.Add([pscustomobject]@{
      ...
      Priority = $f.Priority
      RelatedTask = Format-IdList $immediate 6
      Requirements = Clean-Cell $reqRef
      FlowRef = Clean-Cell $flowRef
      PRDRef = Clean-Cell $prdRef
      ADRRef = Clean-Cell $adrRef
      ...
      KPIRef = Clean-Cell $kpiRef
      ValidationRef = Clean-Cell $validationRef
      EvidenceRef = Clean-Cell $evidenceRef
...
$topAtRisk = $atRisk | Select-Object -First 25
...
[void]$sb.AppendLine('- `n/a` = layer is not required for this feature type')
[void]$sb.AppendLine('- `missing` = required layer has no linked CSV task')
[void]$sb.AppendLine('- `done/in_progress/planned/partial (...)` = layer has linked task coverage')
[void]$sb.AppendLine('')
[void]$sb.AppendLine('## Coverage Snapshot')
[void]$sb.AppendLine('')
[void]$sb.AppendLine('- Route-backed product surfaces: 68 pages (`docs/design/sitemap.md`)')
[void]$sb.AppendLine('- CSV-linked PG tasks covered: 153')
...
[void]$sb.AppendLine('')
[void]$sb.AppendLine('| Group | Feature | Status | Plan Coverage | Purpose | Owner | Priority | Related Task (CSV) | Requirements | Flow Ref | PRD Ref | ADR Ref | Entity | Domain | Database | Adapter | Router | Frontend | KPI Ref | Validation Ref | Evidence Ref | Missing CSV Task(s) | Notes |')
[void]$sb.AppendLine('|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|')
foreach ($m in $sorted) {
  [void]$sb.AppendLine("| $($m.Group) | $($m.Feature) | $($m.Status) | $($m.PlanCoverage) | $($m.Purpose) | $($m.Owner) | $($m.Priority) | $($m.RelatedTask) | $($m.Requirements) | $($m.FlowRef) | $($m.PRDRef) | $($m.ADRRef) | $($m.Entity) | $($m.Domain) | $($m.Database) | $($m.Adapter) | $($m.Router) | $($m.Frontend) | $($m.KPIRef) | $($m.ValidationRef) | $($m.EvidenceRef) | $($m.MissingTasks) | $($m.Notes) |")
}
[void]$sb.AppendLine('')

...
class JulesClient {
  ...
  async startWorkflow(workflowName: string, inputs: Record<string, any>): Promise<AnalysisResult> {
...
describe('Jules Integration', () => {
  ...
  const connectionPath = join(process.cwd(), 'tools/integrations/jules/connection.yaml');
  const workflowPath = join(process.cwd(), 'tools/integrations/jules/workflows/code-analysis.yaml');
  ...
  describe('Workflow Status', () => {
    ...
    it('should handle workflow cancellation', async () => {
      ...
      await expect(client.cancelWorkflow(startResult.workflow_id)).resolves.not.toThrow();
    });
  });
  ...
  describe('Error Handling', () => {
    it('should handle invalid workflow name', async () => {
      await expect(client.startWorkflow('invalid-workflow', {})).rejects.toThrow();
    });

    it('should handle missing required inputs', async () => {
      await expect(client.startWorkflow('code-analysis', {})).rejects.toThrow();
    });
  ...
  describe('Report Generation', () => {
    it('should generate markdown report', async () => {
      ...
      expect(result.reports).toContain(expect.stringContaining('code-analysis'));

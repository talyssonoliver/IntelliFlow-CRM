...
describe('Codex Integration', () => {
  ...
  describe('Configuration', () => {
    ...
    it('should include code-review prompt', () => {
      const config = client.getConfig()
      const codeReviewPrompt = config.prompts.available.find(
        p => p.name === 'code-review'
      )
    ...
    it('should include test-generation prompt', () => {
      const config = client.getConfig()
      const testGenPrompt = config.prompts.available.find(
        p => p.name === 'test-generation'
      )
...
const query = 'SELECT * FROM users WHERE id = ' + userId
      ...
      expect(result.critical_issues.length).toBeGreaterThan(0)
      expect(result.critical_issues.some(
        issue => issue.message.toLowerCase().includes('secret')
      )).toBe(true)
    })
...
function processData(data: any) {
  ...
  describe('Error Handling', () => {
    ...
    it('should handle invalid API key', async () => {
      ...
      await expect(
        invalidClient.reviewCode('const x = 1')
      ).rejects.toThrow()
    })
  ...
  describe('Prompt Management', () => {
    ...
    it('should load test-generation prompt', async () => {
      const promptPath = join(
        process.cwd(),
        'tools/integrations/codex/prompts/test-generation.md'
      )
...
export class Lead {
  email: string // Should be Email value object
      ...
      expect(result.suggestions.some(
        s => s.toLowerCase().includes('value object')
      )).toBe(true)
    })
...
export async function createLead(data: any) {
  throw new Error('Not implemented')
      ...
      expect(result.suggestions.some(
        s => s.toLowerCase().includes('result')
      )).toBe(true)
    })
  })
})

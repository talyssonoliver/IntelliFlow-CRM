...
function main(): void {
  ...
  if (exitCode === 0) {
    if (summary.warnCount > 0) {
      log(`\n[OK] Sprint ${sprintArg.sprint} validations passed (with warnings).`, 'yellow');
      log('     Run with --strict to treat warnings as failures.', 'gray');
    } else {
      log(`\n[OK] Sprint ${sprintArg.sprint} validations passed.`, 'green');
    }
  } else {
    log(`\n[FAIL] Sprint ${sprintArg.sprint} validation failed. See details above.`, 'red');
  }

...
function updateFile(filePath: string, oldStr: string, newStr: string) {
  ...
  if (!content.includes(oldStr)) {
    console.log(
      `[SKIP] Pattern not found in ${path.relative(ROOT, filePath)}`
    );
    return;
  }
  if (DRY_RUN) {
  ...
  if (toAdd.length > 0) {
    ...
    if (updatedContent !== content) {
      if (!DRY_RUN) {
        ...
        console.log(`[DRY RUN] Would update: docs/planning/prd-ai-output-quality.md (+${toAdd.join(", ")})`);
      }
    } else {
...
`;
    ...
    if (readmeContent.includes("## Next Steps")) {
      ...
      readmeChanged = true;
      console.log(DRY_RUN ? "[DRY RUN] Would update: docs/planning/adr/README.md (add retroactive ADR index)" : "[UPDATED] docs/planning/adr/README.md (added retroactive ADR index)");
    } else {
  ...
  if (readmeContent.includes("Future ADRs will use IDs **025+**")) {
    ...
    readmeChanged = true;
    console.log(DRY_RUN ? "[DRY RUN] Would update: docs/planning/adr/README.md (Next Steps section)" : "[UPDATED] docs/planning/adr/README.md (Next Steps section)");
  }
  ...
  if (readmeContent.includes("**Total ADRs**: 25")) {
    ...
    readmeChanged = true;
    console.log(DRY_RUN ? "[DRY RUN] Would update: docs/planning/adr/README.md total count" : "[UPDATED] docs/planning/adr/README.md (total count → 38)");
  }

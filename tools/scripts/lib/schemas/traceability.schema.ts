...
export const capabilitySchema = z.object({
  capability_id: z
    ...
    .describe('Unique identifier for the capability'),
  ...
  domain_services: z
    .array(z.string())
    .optional()
    .describe('File paths to domain service implementations'),
  ...
  sprint_tasks: z
    .array(z.string().regex(taskIdPattern))
    .optional()
    .describe('Task IDs that implement this capability'),
});

// Coverage summary
export const coverageSummarySchema = z.object({
  total_capabilities: z.number().int().min(0).describe('Total number of capabilities defined'),
  with_domain_services: z
    .number()
    .int()
    .min(0)
    .optional()
    .describe('Number of capabilities with domain services'),
  with_api_endpoints: z
    .number()
    .int()
    .min(0)
    .optional()
    .describe('Number of capabilities with API endpoints'),
  with_ui_components: z
    .number()
    .int()
    .min(0)
    .optional()
    .describe('Number of capabilities with UI components'),
  with_tests: z.number().int().min(0).optional().describe('Number of capabilities with tests'),
  traceability_coverage_percentage: z
    .number()
    .min(0)
    .max(100)
    .describe('Percentage of traceability coverage'),
});

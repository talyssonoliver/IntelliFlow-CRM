...
import {
  ...
  collectUniqueCommands,
  runUniqueCommands,
  resolveTaskValidations,
  loadValidationCache,
  isValidationCacheFresh,
  cacheFileToCommandMap,
 ...
 * @param sharedCommandResults - Pre-computed validation command results (command deduplication)
 */
export async function auditTask(
  ...
  sharedCommandResults?: Map<string, ValidationResult>
  ...
  //    Uses shared command cache when available (command deduplication — runs each
  //    unique command once across ALL tasks instead of once per task).
  let validations: Awaited<ReturnType<typeof runTaskValidations>> = [];
  if (!config.skipValidations) {
    if (sharedCommandResults) {
      // Use pre-computed results from the shared command cache
      validations = await resolveTaskValidations(
        taskId,
        task['Validation Method'] || '',
        sharedCommandResults,
        config.repoRoot,
        validationLogsDir,
        config.validationTimeout
      );
    } else {
      // Fallback: run commands directly (standalone task audit, no cache)
  ...
  // 4. Verify KPIs
  //    Skip live measurement when using validation cache (commands already ran)
  //    or when skipValidations is set.
  const skipKpiMeasurement = config.skipValidations || !!sharedCommandResults;
  const kpis = await verifyTaskKpis(taskId, task.KPIs || '', config.repoRoot, skipKpiMeasurement);
...
export async function auditSprintCompletion(
  ...
  // Validation strategy: prefer file-based cache (artifacts/coverage/validation-cache.json)
  // generated by `npx tsx tools/scripts/run-validation-cache.ts`.
  // Falls back to command deduplication (run each unique command once per audit).
  let sharedCommandResults: Map<string, ValidationResult> | undefined;
  if (!fullConfig.skipValidations && completedTasks.length > 0) {
    // 1. Try file-based validation cache first
    const validationCache = loadValidationCache(fullConfig.repoRoot);
    if (validationCache && isValidationCacheFresh(validationCache)) {
      const cacheAge = (
        (Date.now() - new Date(validationCache.generated_at).getTime()) /
        (1000 * 60 * 60)
      ).toFixed(1);
      const cachedCount = Object.keys(validationCache.commands).length;
      const passedCount = Object.values(validationCache.commands).filter((c) => c.passed).length;

      console.log(`Using validation cache (${cacheAge}h old, TTL ${validationCache.ttl_hours}h)`);
      console.log(`  Git: ${validationCache.git_sha} (${validationCache.git_branch})`);
      console.log(`  Cached commands: ${cachedCount} (${passedCount} passed)\n`);

      sharedCommandResults = cacheFileToCommandMap(validationCache);
    } else {
      // 2. Fallback: command deduplication (run each unique command once)
      if (validationCache) {
        console.log('Validation cache found but STALE — falling back to live execution\n');
      } else {
        console.log('No validation cache found — running commands with deduplication\n');
        console.log(
          '  Tip: Run `npx tsx tools/scripts/run-validation-cache.ts` first for faster audits\n'
        );
      }

      const taskCommands = completedTasks.map((t) => ({
        taskId: t['Task ID'],
        validationMethod: t['Validation Method'] || '',
      }));
      const uniqueCommands = collectUniqueCommands(taskCommands);
      const commandList = Array.from(uniqueCommands.keys());

      if (commandList.length > 0) {
        console.log(
          `Command deduplication: ${commandList.length} unique command(s) across ${completedTasks.length} tasks`
        );
        for (const [cmd, taskIds] of uniqueCommands) {
          console.log(`  "${cmd}" → ${taskIds.length} task(s)`);
        }
        console.log('');

        sharedCommandResults = await runUniqueCommands(
          commandList,
          fullConfig.repoRoot,
          outputPaths.validationLogsDir,
          fullConfig.validationTimeout
        );
        console.log('');
      }
    }
  ...
  // Audit each completed task (using cached placeholder scan + shared command results)

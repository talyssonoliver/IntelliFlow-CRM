...
async function runRealDatabaseBenchmarks(): Promise<BenchmarkResult[]> {
  ...
  try {
    ...
    try {
      ...
      console.log(
        `   ⚠️ Skipped findMany with relations: ${e instanceof Error ? e.message.split('\n')[0] : e}`
      );
    }
...
async function runBenchmarks() {
  ...
  if (jsonResult) {
    console.log(
      `  JSON serialization avg: ${jsonResult.avgTime.toFixed(3)}ms (minimal overhead) ✅`
    );
  }
  ...
  console.log(
    `  Database benchmarks:  ${dbResults2.length} ${dbResults2.length > 0 ? (dbPassed ? '✅' : '❌') : '(skipped)'}`
  );
  console.log(
    `  tRPC benchmarks:      ${trpcResults2.length} ${trpcResults2.length > 0 ? (trpcPassed ? '✅' : '❌') : '(skipped)'}`
  );

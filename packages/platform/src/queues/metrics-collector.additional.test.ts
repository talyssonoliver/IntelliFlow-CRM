...
describe('JobMetricsCollector additional', () => {
  ...
  beforeEach(() => {
    c = new JobMetricsCollector('test-q', 60000, 100);
  });
  ...
  describe('recordWaitTime', () => {
    it('records samples', () => {
      c.recordWaitTime(100);
      c.recordWaitTime(300);
  ...
  describe('percentile', () => {
    it('0 for empty', () => {
      expect(c.getMetrics().latency.p95ProcessTimeMs).toBe(0);
    });
    it('p95 correct', () => {
  ...
  describe('error categorization', () => {
    it.each([
      ...
      c.recordError(msg);
      expect(c.getErrorBreakdown()[cat]).toBe(1);
    });
  });
  ...
  describe('pruning', () => {
    it('prunes old data', () => {
      return new Promise<void>((resolve) => {
        ...
        setTimeout(() => {
          expect(sc.getMetrics().processingRates.completedPerMinute).toBe(0);
          resolve();
        }, 150);
      });
    });
  ...
  it('includes queue name', () => {
    expect(c.getMetrics().queueName).toBe('test-q');
  });
});

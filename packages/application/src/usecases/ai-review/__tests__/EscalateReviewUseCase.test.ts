/**
 * EscalateReviewUseCase Tests (IFC-177)
 *
 * Tests for escalating reviews to higher authority.
...
describe('EscalateReviewUseCase', () => {
  let repository: MockAIOutputReviewRepository;
  let eventBus: MockEventBus;
  let useCase: EscalateReviewUseCase;
  ...
  describe('Happy Path', () => {
    ...
    it('should increment escalation depth', async () => {
      const lockToken = generateLockToken();
      const review = createClaimedReview('user-456', lockToken);
      review.clearDomainEvents();
      repository.setReview(review);

      const initialDepth = review.escalationDepth;
      ...
      expect(savedReview.escalationDepth).toBe(initialDepth + 1);
      expect(savedReview.status).toBe(ReviewStatus.ESCALATED);
    });
  });
  ...
  describe('Error Cases', () => {
    ...
    it('should fail when max escalation reached', async () => {
      const lockToken = generateLockToken();
      const review = createClaimedReview('user-456', lockToken);
    ...
    it('should fail when review not in IN_REVIEW status', async () => {
      ...
      const input: EscalateReviewInput = {
        ...
        userId: 'reviewer',
        lockToken: generateLockToken(),
        reason: 'Try to escalate approved.',
      };
  ...
  describe('Concurrency', () => {
    it('should handle escalation race', async () => {
      const lockToken = generateLockToken();
      const review = createClaimedReview('user-456', lockToken);
      review.clearDomainEvents();
      repository.setReview(review);
      ...
      // Second escalation fails (status changed, new lock needed)

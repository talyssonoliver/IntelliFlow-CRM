 ...
 *
 * Moved from apps/api to packages/application because it depends exclusively
 * on application-layer types (CalendarSyncServicePort, WebhookPayload) and
 * is consumed by both apps/api (tRPC router) and apps/web (Next.js route
 * handlers). Keeping it in apps/api created a cross-app boundary violation.
 */

import type { CalendarSyncServicePort, WebhookPayload } from '../ports';

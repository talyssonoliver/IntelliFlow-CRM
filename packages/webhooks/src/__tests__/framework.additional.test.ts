...
import { WebhookFramework, createWebhookFramework, hmacSha256Verify, defaultEventTransformer, stripeEventTransformer, sendgridEventTransformer, type WebhookSourceConfig } from "../framework";
import { createHmac } from "crypto";

function makeSource(ov: Partial<WebhookSourceConfig> = {}): WebhookSourceConfig {
  return { name: ov.name ?? "test-src", secret: ov.secret ?? "secret123",
    signatureHeader: ov.signatureHeader ?? "x-signature",
    signatureVerifier: ov.signatureVerifier ?? hmacSha256Verify,
    enabled: ov.enabled ?? true, allowedEvents: ov.allowedEvents,
...
describe("WebhookFramework additional", () => {
  ...
  beforeEach(() => { fw = createWebhookFramework({ metricsEnabled: true, loggingEnabled: false }); });
  ...
  describe("retry processing", () => {
    ...
    it("retries failed events", async () => {
      ...
      let callCount = 0;
      fw.on("retry.evt", async () => { callCount++; if (callCount === 1) throw new Error("fail once"); });
  ...
  describe("metrics tracking", () => {
    it("tracks failed events", async () => {
      fw.registerSource(makeSource({ name: "s" }));
      fw.on("fail.evt", async () => { throw new Error("boom"); });
  ...
  describe("custom event transformer", () => {
    it("uses source eventTransformer", async () => {
      const transformer = (raw: unknown) => {
        const d = raw as any;
        return { id: d.custom_id, type: d.custom_type, source: "custom", timestamp: new Date(), version: "1.0", payload: d };
      };

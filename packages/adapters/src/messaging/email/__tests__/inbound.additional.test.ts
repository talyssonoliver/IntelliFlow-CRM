...
describe("SpamAnalyzer - content patterns", () => {
  ...
  it("should detect suspicious links in HTML body", () => {
    ...
    const email = {
      ...
      htmlBody: '<a href="https://bit.ly/abc123">Click here</a> <a href="https://tinyurl.com/xyz">And here</a>',
      attachments: [],
      rawSize: 200,
      parsedAt: new Date(),
      isReply: false,
...
describe("Forward detection", () => {
  ...
  it("should detect forwarded message body indicators", () => {
    expect(isForwardedMessage("Regular subject", "---------- Forwarded message ---------")).toBe(true);
...
describe("Reply detection", () => {
  it("should detect Re: prefix", () => {
    const headers = { subject: "Re: Previous email", from: { address: "a@b.com", raw: "" }, to: [] };
    expect(isReplyMessage("Re: Previous email", headers as any)).toBe(true);
  });

  it("should detect AW: prefix (German)", () => {
    const headers = { subject: "AW: Vorherige E-Mail", from: { address: "a@b.com", raw: "" }, to: [] };
  ...
  it("should detect reply by In-Reply-To header", () => {
    const headers = { subject: "No prefix", from: { address: "a@b.com", raw: "" }, to: [], inReplyTo: "msg-123" };
    expect(isReplyMessage("No prefix", headers as any)).toBe(true);
  });

  it("should detect reply by references", () => {
    const headers = { subject: "No prefix", from: { address: "a@b.com", raw: "" }, to: [], references: ["ref-1"] };
    expect(isReplyMessage("No prefix", headers as any)).toBe(true);
  });
});

...
describe('payment-intents handler', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockMap.mockReturnValue(mockPI);
  });
  ...
  describe('confirmPaymentIntent', () => {
    ...
    it('should confirm with payment method', async () => {
      ...
      expect((mockMakeRequest.mock.calls[0][3] as URLSearchParams).get('payment_method')).toBe(
        'pm_456'
      );
    });
    ...
    it('should handle non-Error thrown', async () => {
      ...
      expect(((await confirmPaymentIntent(config, 'pi_123')).error as any).message).toBe(
        'Unknown error'
      );
    });
  });
  ...
  describe('capturePaymentIntent', () => {
    ...
    it('should capture with amount', async () => {
      ...
      expect((mockMakeRequest.mock.calls[0][3] as URLSearchParams).get('amount_to_capture')).toBe(
        '3000'
      );
    });
  ...
  describe('cancelPaymentIntent', () => {
    ...
    it('should handle thrown non-Error', async () => {
      ...
      expect(((await cancelPaymentIntent(config, 'pi_123')).error as any).message).toBe(
        'Unknown error'
      );
    });
  });
  ...
  describe('getPaymentIntent', () => {
    ...
    it('should handle thrown non-Error', async () => {
      ...
      expect(((await getPaymentIntent(config, 'pi_123')).error as any).message).toBe(
        'Unknown error'
      );
    });
  });

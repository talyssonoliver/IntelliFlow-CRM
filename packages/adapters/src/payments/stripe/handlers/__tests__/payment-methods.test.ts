...
const mockPM: StripePaymentMethod = {
  id: 'pm_123',
  type: 'card',
  customerId: 'cus_123',
  card: { brand: 'visa', last4: '4242', expMonth: 12, expYear: 2026, funding: 'credit' },
...
describe('payment-methods handler', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockMapToPM.mockReturnValue(mockPM);
  });
  ...
  describe('attachPaymentMethod', () => {
    ...
    it('should handle thrown Error', async () => {
      ...
      expect(((await attachPaymentMethod(config, 'pm_123', 'cus_123')).error as any).message).toBe(
        'Network error'
      );
    });
  ...
  describe('detachPaymentMethod', () => {
    ...
    it('should handle thrown non-Error', async () => {
      ...
      expect(((await detachPaymentMethod(config, 'pm_123')).error as any).message).toBe(
        'Unknown error'
      );
    });
  });
  ...
  describe('listPaymentMethods', () => {
    ...
    it('should handle thrown non-Error', async () => {
      ...
      expect(((await listPaymentMethods(config, 'cus_123')).error as any).message).toBe(
        'Unknown error'
      );
    });
  });

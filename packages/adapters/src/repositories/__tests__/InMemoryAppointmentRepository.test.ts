...
describe('InMemoryAppointmentRepository', () => {
  ...
  beforeEach(() => { repo = new InMemoryAppointmentRepository(); });

  it('save/findById', async () => {
    const a = makeApt({ id: 'a1' }); await repo.save(a);
  ...
  it('findByOrganizer filters and sorts', async () => {
    const t1=new Date(2025,0,1),t2=new Date(2025,0,2);
    ...
    expect(r).toHaveLength(2); expect(r[0].id.value).toBe('o2');
  });
  it('findByOrganizer paginates', async () => {
    for(let i=0;i<5;i++) await repo.save(makeApt({id:'p'+i,organizerId:'P',startTime:new Date(2025,0,i+1)}));
  ...
  it('findInTimeRange', async () => {
    await repo.save(makeApt({id:'tr1',startTime:new Date(2025,0,10,9,30),endTime:new Date(2025,0,10,10,30)}));
    await repo.save(makeApt({id:'tr2',startTime:new Date(2025,0,10,12,0),endTime:new Date(2025,0,10,13,0)}));
    const r=await repo.findInTimeRange(new Date(2025,0,10,10,0),new Date(2025,0,10,11,0));
    expect(r).toHaveLength(1); expect(r[0].id.value).toBe('tr1');
  });
  it('findOverlapping excludes cancelled/no-show', async () => {
    const s=new Date(2025,1,1,10,0),e=new Date(2025,1,1,11,0);
    ...
    expect(r).toHaveLength(1); expect(r[0].id.value).toBe('ov3');
  });
  it('findOverlapping excludes by id', async () => {
    const s=new Date(2025,1,2,10,0),e=new Date(2025,1,2,11,0);
    await repo.save(makeApt({id:'ov4',status:'SCHEDULED',startTime:s,endTime:e}));
    expect(await repo.findOverlapping({startTime:s,endTime:e} as any,{value:'ov4'} as any)).toHaveLength(0);
  });
  ...
  it('findWithFilters by appointmentType', async () => {
    ...
    expect((await repo.findWithFilters({appointmentType:['MEETING'] as any})).items).toHaveLength(1);
  });
  it('findWithFilters by time range', async () => {
    ...
    expect((await repo.findWithFilters({startTimeFrom:new Date(2025,0,5),startTimeTo:new Date(2025,0,15)})).items).toHaveLength(1);
  });
  ...
  it('findWithFilters hasMore', async () => {
    ...
    expect(r.hasMore).toBe(true); expect(r.total).toBe(3);
  });
  ...
  it('countByStatus all', async () => {
    ...
    expect(c2.SCHEDULED).toBe(2); expect(c2.COMPLETED).toBe(1); expect(c2.CANCELLED).toBe(0);
  });
  ...
  it('findUpcoming future non-cancelled', async () => {
    ...
    await repo.save(makeApt({id:'up1',organizerId:'u1',startTime:future,status:'SCHEDULED'}));
    await repo.save(makeApt({id:'up2',organizerId:'u1',startTime:past,status:'SCHEDULED'}));
    await repo.save(makeApt({id:'up3',organizerId:'u1',startTime:future,status:'CANCELLED'}));
    const r=await repo.findUpcoming('u1');
    expect(r).toHaveLength(1); expect(r[0].id.value).toBe('up1');
  });
  it('findUpcoming respects limit', async () => {
    const f=new Date(Date.now()+86400000);
    for(let i=0;i<5;i++) await repo.save(makeApt({id:'ul'+i,organizerId:'u2',startTime:new Date(f.getTime()+i*60000),status:'CONFIRMED'}));
    expect(await repo.findUpcoming('u2',2)).toHaveLength(2);
  });
  it('findPast sorted desc', async () => {
    const p1=new Date(Date.now()-172800000),p2=new Date(Date.now()-86400000);
    await repo.save(makeApt({id:'pa1',organizerId:'u3',startTime:p1,endTime:new Date(p1.getTime()+3600000)}));
    await repo.save(makeApt({id:'pa2',organizerId:'u3',startTime:p2,endTime:new Date(p2.getTime()+3600000)}));
    await repo.save(makeApt({id:'pa3',organizerId:'u3',startTime:new Date(Date.now()+86400000),endTime:new Date(Date.now()+90000000)}));
    const r=await repo.findPast('u3');
    expect(r).toHaveLength(2); expect(r[0].id.value).toBe('pa2');
  });
  it('findByExternalCalendarId finds', async () => {
    ...
    expect(f).toBeTruthy(); expect(f!.id.value).toBe('ec1');
  });
  ...
  it('hasConflicts with organizer', async () => {
    const s=new Date(2025,2,1,10,0),e=new Date(2025,2,1,11,0);
    await repo.save(makeApt({id:'hc1',organizerId:'u4',startTime:s,endTime:e,status:'SCHEDULED'}));
    expect(await repo.hasConflicts({startTime:s,endTime:e} as any,['u4'])).toBe(true);
  });
  it('hasConflicts with attendee', async () => {
    const s=new Date(2025,2,2,10,0),e=new Date(2025,2,2,11,0);
    await repo.save(makeApt({id:'hc2',organizerId:'x',attendeeIds:['u5'],startTime:s,endTime:e,status:'CONFIRMED'}));
    expect(await repo.hasConflicts({startTime:s,endTime:e} as any,['u5'])).toBe(true);
  });
  it('hasConflicts false cancelled', async () => {
    const s=new Date(2025,2,3,10,0),e=new Date(2025,2,3,11,0);
    await repo.save(makeApt({id:'hc3',organizerId:'u6',startTime:s,endTime:e,status:'CANCELLED'}));
    expect(await repo.hasConflicts({startTime:s,endTime:e} as any,['u6'])).toBe(false);
  });
  it('hasConflicts excludes by id', async () => {
    const s=new Date(2025,2,4,10,0),e=new Date(2025,2,4,11,0);
    await repo.save(makeApt({id:'hc4',organizerId:'u7',startTime:s,endTime:e,status:'SCHEDULED'}));
    expect(await repo.hasConflicts({startTime:s,endTime:e} as any,['u7'],{value:'hc4'} as any)).toBe(false);
  });
  it('hasConflicts no conflict non-attendee', async () => {
    const s=new Date(2025,2,5,10,0),e=new Date(2025,2,5,11,0);
    await repo.save(makeApt({id:'hc5',organizerId:'someone',startTime:s,endTime:e,status:'SCHEDULED'}));
    expect(await repo.hasConflicts({startTime:s,endTime:e} as any,['other'])).toBe(false);
  });
  it('findForConflictCheck', async () => {
    await repo.save(makeApt({id:'fcc1',organizerId:'u8',startTime:new Date(2025,3,1,10,0),endTime:new Date(2025,3,1,11,0),status:'SCHEDULED'}));
    expect(await repo.findForConflictCheck(['u8'],{startTime:new Date(2025,3,1,9,0),endTime:new Date(2025,3,1,12,0)})).toHaveLength(1);
  });
  ...
  it('batchUpdateStatus no error', async () => {
    ...
    await expect(repo.batchUpdateStatus([{value:'bu1'} as any],'COMPLETED' as any)).resolves.toBeUndefined();
  });
  it('findNeedingReminder', async () => {
    const soon=new Date(Date.now()+10*60000);
    await repo.save(makeApt({id:'rn1',startTime:soon,status:'SCHEDULED',reminderMinutes:30}));
    await repo.save(makeApt({id:'rn2',startTime:soon,status:'COMPLETED',reminderMinutes:30}));
    await repo.save(makeApt({id:'rn3',startTime:soon,status:'CONFIRMED',reminderMinutes:null}));
    const r=await repo.findNeedingReminder(30);
    expect(r).toHaveLength(1); expect(r[0].id.value).toBe('rn1');
  });
  it('clear/count/getAll', async () => {
    await repo.save(makeApt({id:'th1'}));
    await repo.save(makeApt({id:'th2'}));
    expect(repo.count()).toBe(2); expect(repo.getAll()).toHaveLength(2);
    repo.clear(); expect(repo.count()).toBe(0);
  });
});

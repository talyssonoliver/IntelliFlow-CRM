...
function makeTask(ov: Partial<{
  id: string; title: string; status: string; priority: string;
  ownerId: string; leadId: string; contactId: string;
  opportunityId: string; dueDate: Date; createdAt: Date;
}> = {}): Task {
  const id = TaskId.create(ov.id ?? uuid()).value as TaskId;
  return Task.reconstitute(id, {
    title: ov.title ?? "Test", status: (ov.status ?? "PENDING") as any,
    priority: (ov.priority ?? "MEDIUM") as any, ownerId: ov.ownerId ?? "owner-1",
    tenantId: "t-1", leadId: ov.leadId, contactId: ov.contactId,
    opportunityId: ov.opportunityId, dueDate: ov.dueDate,
    createdAt: ov.createdAt ?? new Date("2025-01-01"), updatedAt: new Date(),
  } as any);
}
...
describe("InMemoryTaskRepository", () => {
  ...
  beforeEach(() => { repo = new InMemoryTaskRepository(); counter = 200; });

  describe("save/findById", () => {
    it("saves and retrieves", async () => {
      const t = makeTask({ id: ID1 }); await repo.save(t);
  ...
  describe("findByOwnerId", () => {
    ...
    it("empty for unknown", async () => { expect(await repo.findByOwnerId("x")).toEqual([]); });
  });
  ...
  describe("clear/getAll", () => {
    it("clear empties", async () => {
      await repo.save(makeTask()); await repo.clear();

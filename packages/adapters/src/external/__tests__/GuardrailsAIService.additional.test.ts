...
describe('GuardrailsAIService additional', () => {
  ...
  beforeEach(() => {
    ...
    (mockAI.scoreLead as any).mockResolvedValue(
      Result.ok({
        score: 75,
        confidence: 0.85,
        modelVersion: 'v1',
    ...
    (mockAudit.logSecurityEvent as any).mockResolvedValue({
      eventId: 'e1',
  ...
  describe('generateEmail', () => {
    ...
    it('handles inner service failure', async () => {
      (mockAI.generateEmail as any).mockResolvedValue(
        Result.fail({ message: 'fail', code: 'ERR' })
      );
  ...
  describe('scoreLead with bias detection', () => {
    it('collects bias metrics when enabled', async () => {
      const biasService = new GuardrailsAIService(mockAI, mockAudit, {
        ...defaultConfig,
        enableBiasDetection: true,
      });
      (mockAI.scoreLead as any).mockResolvedValue(
        Result.ok({ score: 80, confidence: 0.9, modelVersion: 'v1' } as LeadScoringResult)
      );
  ...
  describe('analyzeBiasMetrics', () => {
    ...
    it('logs when bias detected', async () => {
      mockDetectScoreBias.mockReturnValue({
        biasDetected: true,
        violations: [{ segment: 'domain', severity: 'high' }],
      });
      const biasService = new GuardrailsAIService(mockAI, mockAudit, {
        ...defaultConfig,
        enableBiasDetection: true,
    ...
    it('redacts email from reasoning', async () => {
      (mockAI.scoreLead as any).mockResolvedValue(
        Result.ok({
          ...
          confidence: 0.9,
          modelVersion: 'v1',
          reasoning: 'User john@example.com scored high',
        } as LeadScoringResult)
      );
      const r = await service.scoreLead(input);
      expect(r.isSuccess).toBe(true);
  ...
  describe('logging disabled', () => {
    it('does not log when enableLogging is false', async () => {
      const noLogService = new GuardrailsAIService(mockAI, mockAudit, {
        ...defaultConfig,
        enableLogging: false,
      });

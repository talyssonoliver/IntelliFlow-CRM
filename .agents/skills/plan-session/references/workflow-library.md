...
**IMPORTANT**: This command uses the shared workflow library for consistent behavior with the UI.
...
The shared library ensures both `/plan-session` CLI and UI "SESSION 2: Plan" button use:
...
This command supports an optional peer reviewer teammate when agent teams are enabled.
...
**Detection logic**: Plan session always defaults to subagent mode. Agent team mode is opt-in — the lead checks the complexity threshold AFTER decomposing TDD steps. If the threshold is met and teams are enabled, a reviewer is spawned.

**Token cost warning**: Spawning a Plan-Reviewer adds ~1x additional token cost (single teammate). Only triggers for genuinely complex plans.

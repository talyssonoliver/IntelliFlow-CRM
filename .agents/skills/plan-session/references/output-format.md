...
Plan is written to the sprint-based directory.
...
- **Files to Create:** and **Files to Modify:** MUST use plural block format with newline + list:
  ```
  **Files to Create:**
  - `path/to/file.ts`
  ```
...
- `## Validation Matrix` is mandatory and must use executable commands, not vague prose

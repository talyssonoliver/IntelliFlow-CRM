...
- `status`: string — UPPERCASE (`BACKLOG`, `PLANNED`, `DONE`, `IN_PROGRESS`,
  etc.)
- `dependencies`: array of string — task IDs this task depends on
- `dependents`: array of string — task IDs that depend on this task (populated
  by reverse edges)
...
For each dependency, add the new task ID to that dependency's `dependents`
array.
...
3. If the new task ID appears anywhere in the chain → CIRCULAR — abort and
   report to user
...
If any dependency is in a different sprint than the new task, add an entry to
the `cross_sprint_dependencies` array:

...
3. If a related document exists, **link to it** in the CSV Pre-requisites column
...
Naming: lowercase, hyphenated slug from the feature name. Example:
`prd-contact-merge.md`
...
The full PRD template is at `docs/planning/prd-template.md` — the stub is
intentionally minimal. The task implementer will flesh it out during
spec-session.
...
Format: `ADR-{NNN}-<slug>.md` where NNN is zero-padded to 3 digits and slug is
lowercase-hyphenated. Example: `ADR-038-contact-merge-strategy.md`

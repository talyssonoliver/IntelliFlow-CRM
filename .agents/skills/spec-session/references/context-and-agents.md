...
Automatically hydrates context if not done. Gathers task metadata, dependencies, codebase patterns.

## Phase 0.5: Agent Selection

Analyzes task domain and selects 3-5 agents. Always includes: Domain-Expert, Test-Engineer.
...
> **STOP** — You MUST invoke `/spec-exploration <TASK_ID>` before any analysis rounds.
> Each agent MUST use tools (Read, Grep, Glob) to verify code before reasoning about it.
> See `/spec-exploration` for exploration templates and verification checklist.

## CRITICAL: Metrics Recording

At session **START**, record to task JSON at `apps/project-tracker/docs/metrics/sprint-{N}/phase-*/TASK-ID.json`:
- `started_at`, `status: "Specifying"`, `status_history` entry, `executor`, `agents`

At session **END**, record:
- `status: "Spec Complete"`, `status_history` entry, `artifacts.created` with SHA256 hash

## Requirements Flow

- Performs **Analise de Requisitos**: document user/domain needs, constraints, regulations, existing systems.
- **PRD/ADR resolution is handled in Phase 0.97** — see `references/prd-adr-resolution.md`.

...
description:
  'Iterates through all tasks marked "Completed" that have integrity failures
  (unchecked plan steps, missing attestation, incomplete validations, missing
  files). For each task: reads the plan, verifies code against every step,
  implements what''s missing, runs the 4 mandatory validations, and creates
  proper attestation.'
---
...
- NEVER fabricate validation results — run actual commands, capture real exit
  codes
...
- Attestation schema:
  `apps/project-tracker/docs/metrics/schemas/attestation.schema.json`

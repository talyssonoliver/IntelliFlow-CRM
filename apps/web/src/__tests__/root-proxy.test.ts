...
const mockRedirect = vi.fn().mockImplementation((url: any) => ({
  type: 'redirect', url: url.toString(), headers: new Map(), cookies: { delete: vi.fn() },
}));
const mockNextFn = vi.fn().mockImplementation(() => ({
  type: 'next', headers: { set: vi.fn() }, cookies: { delete: vi.fn() },
}));

vi.mock('next/server', () => ({
  NextRequest: class {},
  NextResponse: { redirect: (...a: any[]) => mockRedirect(...a), next: (...a: any[]) => mockNextFn(...a) },
}));

import { proxy, config } from '../../proxy';

function mkReq(path: string, opts: { accessToken?: string; session?: string; searchParams?: Record<string,string> } = {}) {
  const url = new URL(`http://localhost:3000${path}`);
  if (opts.searchParams) Object.entries(opts.searchParams).forEach(([k,v]) => url.searchParams.set(k,v));
...
describe('root proxy.ts', () => {
  ...
  describe('static bypass', () => {
    it('passes /_next', async () => { await proxy(mkReq('/_next/static/c.js')); expect(mockNextFn).toHaveBeenCalled(); });
    it('passes /api', async () => { await proxy(mkReq('/api/trpc')); expect(mockNextFn).toHaveBeenCalled(); });
    it('passes files with extension', async () => { await proxy(mkReq('/favicon.ico')); expect(mockNextFn).toHaveBeenCalled(); });
  });
  ...
  describe('config', () => {
    it('has matcher', () => { expect(config.matcher).toBeDefined(); expect(config.matcher[0]).toContain('api'); });
  });
});

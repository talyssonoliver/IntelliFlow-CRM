 ...
 * subscriber management, and health checks.
 */
...
describe('incident-tracker', () => {
  ...
  describe('updateServiceStatus', () => {
    ...
    it('should return undefined for nonexistent service', () => {
      expect(updateServiceStatus('nonexistent', 'operational')).toBeUndefined();
    });
  });
  ...
  describe('getOverallStatus', () => {
    ...
    it('should return operational when no services exist', () => {
      const result = getOverallStatus();
      expect(result.status).toBe('operational');
      expect(result.affectedCount).toBe(0);
    });
  ...
  describe('createIncident', () => {
    ...
    it('should update affected service to major_outage for critical severity', () => {
      createIncident('Major Issue', 'critical', ['api'], 'Major outage');
      expect(getServiceById('api')?.status).toBe('major_outage');
    });

    it('should update affected service to partial_outage for major severity', () => {
      createIncident('Partial Issue', 'major', ['api'], 'Partial outage');
      expect(getServiceById('api')?.status).toBe('partial_outage');
    });

    it('should update affected service to degraded for minor severity', () => {
      createIncident('Minor Issue', 'minor', ['api'], 'Degraded');
      expect(getServiceById('api')?.status).toBe('degraded');
    });

    it('should update affected service to degraded for maintenance severity', () => {
      createIncident('Maintenance', 'maintenance', ['api'], 'Scheduled maintenance');
      expect(getServiceById('api')?.status).toBe('degraded');
    });

    it('should update multiple affected services', () => {
      createIncident('Multi-Service', 'major', ['api', 'web'], 'Both affected');
      expect(getServiceById('api')?.status).toBe('partial_outage');
  ...
  describe('addIncidentUpdate', () => {
    ...
    it('should keep existing status when not provided', () => {
      const result = addIncidentUpdate(incidentId, 'Update without status change');
    ...
    it('should set resolvedAt when status is completed', () => {
      const result = addIncidentUpdate(incidentId, 'Maintenance completed', 'completed');
    ...
    it('should return undefined for nonexistent incident', () => {
      expect(addIncidentUpdate('nonexistent', 'Update')).toBeUndefined();
    });
  });
  ...
  describe('getActiveIncidents', () => {
    ...
    it('should return empty array when all resolved', () => {
      const inc = createIncident('Test', 'minor', ['api'], 'Initial');
      addIncidentUpdate(inc.id, 'Done', 'resolved');
  ...
  describe('getRecentIncidents', () => {
    ...
    it('should accept custom days parameter', () => {
      ...
      expect(getRecentIncidents(7)).toHaveLength(0);
    });

    it('should include incidents within custom range', () => {
      createIncident('Recent', 'minor', ['api'], 'Recent');
      vi.advanceTimersByTime(5 * 24 * 60 * 60 * 1000);

      expect(getRecentIncidents(7)).toHaveLength(1);
    });
  });
  ...
  describe('subscribe', () => {
    ...
    it('should set services to undefined when not provided', () => {
      const subscriber = subscribe('test@example.com');
      expect(subscriber.services).toBeUndefined();
    });
  });
  ...
  describe('unsubscribe', () => {
    it('should remove subscriber and return true', () => {
      const subscriber = subscribe('test@example.com');
      expect(unsubscribe(subscriber.id)).toBe(true);
    });

    it('should return false for nonexistent subscriber', () => {
      expect(unsubscribe('nonexistent')).toBe(false);
    });
  });

  describe('getSubscribersForService', () => {
    beforeEach(() => {
      subscribe('all@example.com');
    ...
    it('should return subscribers with no service filter (subscribed to all)', () => {
    ...
    it('should not return subscribers for other services only', () => {
      const subscribers = getSubscribersForService('api');
      const emails = subscribers.map((s) => s.email);
      expect(emails).not.toContain('web@example.com');
    });

    it('should return subscribers with empty services array (subscribed to all)', () => {
      subscribe('empty@example.com', []);
      vi.advanceTimersByTime(1);

      const subscribers = getSubscribersForService('any-service');
      const emails = subscribers.map((s) => s.email);
      expect(emails).toContain('empty@example.com');
    });
  });
  ...
  describe('calculateUptime', () => {
    ...
    it('should use default days parameter', () => {
      const uptime = calculateUptime('api');
      expect(uptime).toBeGreaterThanOrEqual(99.9);
      expect(uptime).toBeLessThanOrEqual(100);
    });
  });
  ...
  describe('healthCheck', () => {
    ...
    it('should return healthy=true for successful fast response', async () => {
    ...
    it('should return healthy=false for failed requests', async () => {
    ...
    it('should set partial_outage for very slow healthy response', async () => {
      globalThis.fetch = vi.fn().mockImplementation(async () => {
        await new Promise((resolve) => {
  ...
  describe('resetStore', () => {
    it('should clear all services, incidents, and subscribers', () => {
      initializeServices([
        { id: 'api', name: 'API', description: 'Desc', status: 'operational', uptime: 99.9 },
      ]);
      createIncident('Test', 'minor', ['api'], 'Test');
      ...
      expect(getAllServices()).toEqual([]);
      expect(getAllIncidents()).toEqual([]);
      expect(getSubscribersForService('api')).toEqual([]);
    });
  });
});

...
import {
  TIMELINE_EVENT_TYPES, TIMELINE_PRIORITIES, COMMUNICATION_CHANNELS,
  AGENT_ACTION_STATUSES, DOCUMENT_STATUSES,
  groupEventsByDate, getEventTypeIcon, getEventTypeColor,
  type TimelineEvent, type TimelineFilters, type TimelineQueryOptions,
  type TimelineResponse, type TimelineStats,
...
describe('timeline-event-model', () => {
  describe('constants', () => {
    ...
    it('TIMELINE_PRIORITIES', () => expect(TIMELINE_PRIORITIES).toEqual(['low','medium','high','urgent']));
    ...
    it('AGENT_ACTION_STATUSES', () => expect(AGENT_ACTION_STATUSES).toEqual(['pending_approval','approved','rejected','rolled_back','expired']));
    it('DOCUMENT_STATUSES', () => expect(DOCUMENT_STATUSES).toEqual(['draft','under_review','approved','signed','archived','superseded']));
  });
  ...
  describe('groupEventsByDate', () => {
    ...
    it('uses date format for older dates', () => {
      const g = groupEventsByDate([{ id:'1', type:'note', title:'N', timestamp: new Date('2025-06-15') }]);
  ...
  describe('getEventTypeIcon', () => {
    ...
    it('task_completed -> check_circle', () => expect(getEventTypeIcon('task_completed')).toBe('check_circle'));
    ...
    it('agent_action -> smart_toy', () => expect(getEventTypeIcon('agent_action')).toBe('smart_toy'));
    it('escalation -> priority_high', () => expect(getEventTypeIcon('escalation')).toBe('priority_high'));
  ...
  describe('getEventTypeColor', () => {
    ...
    it('task_completed -> text-green-500', () => expect(getEventTypeColor('task_completed')).toBe('text-green-500'));
    it('task_overdue -> text-red-500', () => expect(getEventTypeColor('task_overdue')).toBe('text-red-500'));
    it('email -> text-sky-500', () => expect(getEventTypeColor('email')).toBe('text-sky-500'));
    it('escalation -> text-red-600', () => expect(getEventTypeColor('escalation')).toBe('text-red-600'));
    it('unknown -> text-slate-500', () => expect(getEventTypeColor('unknown' as any)).toBe('text-slate-500'));
  });
  ...
  describe('type interfaces', () => {
    ...
    it('TimelineStats', () => {
      const s: TimelineStats = { totalEvents:100, byType:{ task:30 } as any, pendingApprovals:5, overdueTasks:2 };

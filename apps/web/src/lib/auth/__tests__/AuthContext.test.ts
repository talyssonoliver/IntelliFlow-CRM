...
describe('AuthContext - JWT Utility Functions', () => {
  ...
  describe('tokenNeedsRefresh', () => {
    ...
    it('custom threshold', () => {
      expect(tokenNeedsRefresh(createTestJwt(Math.floor(Date.now() / 1000) + 120), 60000)).toBe(
        false
      );
    });
    it('true for invalid', () => {
      expect(tokenNeedsRefresh('not-jwt')).toBe(true);
    });
  });
...
describe('AuthContext - State Machine', () => {
  let state: AuthState;
  function setState(u: AuthState | ((p: AuthState) => AuthState)) {
  ...
  describe('login', () => {
    ...
    it('MFA required', () => {
      setState((p) => ({
        ...p,
        isLoading: false,
        mfa: { required: true, challengeId: 'ch-xyz', methods: ['totp', 'sms'] },
      }));
  ...
  describe('logout', () => {
    it('clears all state', () => {
      state = {
        ...
        session: { accessToken: 'tok', expiresAt: new Date() },
        isAuthenticated: true,
        isLoading: false,
        mfa: { required: false, challengeId: null, methods: [] },
        error: null,
  ...
  describe('MFA verification', () => {
    ...
    it('authenticated after MFA', () => {
      ...
      setState((p) => ({
        ...p,
        user: { id: 'u1', email: 'a@b.com', name: 'A', role: 'USER' },
        session: { accessToken: 'mfa-tok', expiresAt: new Date() },
        isAuthenticated: true,
        isLoading: false,
        mfa: { required: false, challengeId: null, methods: [] },
      }));
  ...
  describe('status query', () => {
    it('sets user from query', () => {
      setState((p) => ({
        ...p,
        user: { id: 'u1', email: 't@e.com', name: 'T', role: 'ADMIN' },
        isAuthenticated: true,
        isLoading: false,
        session: { accessToken: '', expiresAt: new Date(Date.now() + 3600000) },
      }));
      expect(state.isAuthenticated).toBe(true);
  ...
  describe('utilities', () => {
    ...
    it('setMfaRequired', () => {
      setState((p) => ({
        ...p,
        mfa: { required: true, challengeId: 'ext-123', methods: ['totp', 'email'] },
      }));
      expect(state.mfa.required).toBe(true);

...
/**
 * Taint-breaking ADR filename builder.
 * `path.basename` collapses any directory traversal; `nextId` is validated to
 * exactly 3 digits; the title slug is restricted to `[a-z0-9-]`. The result is
...
function _buildSafeAdrFilename(nextId: string, title: string): string | null {
  ...
  // `path.basename` is the CodeQL-recognised taint-break.
  return path.basename(`ADR-${nextId}-${slug}.md`);
}

// Configuration
const ADR_PATHS = ['docs/architecture/adr', 'docs/shared'];

// Input validation helpers. The real safety comes from the downstream slug
// sanitization when we build the filename (`[^a-z0-9-]` -> ''); this check
// only guards length bounds and rejects control characters / path separators
// that could confuse the prompt or leak through a logging sink.
function sanitizeAdrTitle(title: string): string | null {
  if (typeof title !== 'string') return null;
  const trimmed = title.trim();
  if (trimmed.length === 0 || trimmed.length > 200) return null;
  // Reject any Unicode control character (Cc)
...
/**
 * Containment check using `path.relative`, which is separator-agnostic and
 * also survives test doubles that patch `path.resolve`/`join` with POSIX
 * semantics on Windows. A path is "inside" `root` iff the relative form has
 * no leading `..` segment and is not absolute.
 */
...
export function createADR(
  ...
  const safeTitle = sanitizeAdrTitle(title);
  if (!safeTitle) {
  ...
  try {
    ...
    if (!isInsideDir(adrDirAbs, outputPathResolved)) {
      return { success: false, error: 'Refusing to write outside docs/architecture/adr' };
    }
    ...
    template = template.replaceAll('[Title of Decision]', safeTitle);
...
export function updateADRStatus(
  ...
  try {
    ...
    const adrDirAbs = path.resolve(baseDir, 'docs/architecture/adr');
    const sharedDirAbs = path.resolve(baseDir, 'docs/shared');
    if (!isInsideDir(adrDirAbs, fullPath) && !isInsideDir(sharedDirAbs, fullPath)) {
      return { success: false, error: 'Refusing to write outside docs/architecture/adr' };
    }

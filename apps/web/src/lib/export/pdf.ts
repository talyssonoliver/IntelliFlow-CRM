...
/**
 * Export feedback survey report to PDF (IFC-068)
 */
export interface FeedbackReportData {
  ...
  responseRates: Array<{ type: string; sent: number; responded: number; rate: number }>;
}

export function exportFeedbackReportToPDF(
  data: FeedbackReportData,
  options: PDFExportOptions = {}
): void {
  const metrics: Array<{ name: string; value: string | number }> = [
    { name: 'Total Responses', value: data.totalResponses },
  ];

  if (data.npsScore !== null) {
    metrics.push({ name: 'NPS Score', value: data.npsScore });
  }
  if (data.csatScore !== null) {
    metrics.push({ name: 'CSAT Score', value: `${data.csatScore}%` });
  }
  if (data.cesScore !== null) {
    metrics.push({ name: 'CES Score', value: data.cesScore });
  }
  ...
  if (data.sentiment) {
    sections.push({
      ...
      type: 'table',
      data: [
        { Sentiment: 'Positive', Count: data.sentiment.positive },
        { Sentiment: 'Neutral', Count: data.sentiment.neutral },
        { Sentiment: 'Negative', Count: data.sentiment.negative },
      ],
    });
  }

  if (data.responseRates.length > 0) {
    sections.push({
      title: 'Response Rates',
      type: 'table',
      data: data.responseRates.map((rr) => ({
        Type: rr.type,
        Sent: rr.sent,
        Responded: rr.responded,
        'Rate (%)': rr.rate,
      })),
    });
  }
  ...
  exportToPDF(sections, {
    ...
    subtitle: `Period: ${data.dateRange}`,
    ...options,
  });
}

...
const mockChannel = { on: vi.fn().mockReturnThis(), subscribe: vi.fn((cb: any) => { setTimeout(() => cb('SUBSCRIBED'), 5); return mockChannel; }), unsubscribe: vi.fn() };
...
describe('use-subscription supplementary', () => {
  beforeEach(() => {
    ...
    mockChannel.subscribe.mockImplementation((cb: any) => { setTimeout(() => cb('SUBSCRIBED'), 5); return mockChannel; });
    mockSupa.channel.mockReturnValue(mockChannel);
    mockSupa.removeChannel.mockResolvedValue(undefined);
  });

  it('exports useSubscription function', async () => { const m = await import('../../../hooks/use-subscription'); expect(typeof m.useSubscription).toBe('function'); });
  it('exports useLeadScoreSubscription', async () => { const m = await import('../../../hooks/use-subscription'); expect(typeof m.useLeadScoreSubscription).toBe('function'); });
  it('exports useActivitySubscription', async () => { const m = await import('../../../hooks/use-subscription'); expect(typeof m.useActivitySubscription).toBe('function'); });
  it('exports useRealtimeHealth', async () => { const m = await import('../../../hooks/use-subscription'); expect(typeof m.useRealtimeHealth).toBe('function'); });
  it('exports supabase client', async () => { const m = await import('../../../hooks/use-subscription'); expect(m.supabase).toBeDefined(); });
  it('exports REALTIME_BACKEND', async () => { const m = await import('../../../hooks/use-subscription'); expect(['trpc', 'supabase']).toContain(m.REALTIME_BACKEND); });

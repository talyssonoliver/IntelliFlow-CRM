...
const mockQueryData = { items: [{ entityType: 'lead', entityId: 'l1', title: 'Lead 1', url: '/leads/l1' }] };
...
vi.mock('@/lib/trpc', () => ({
  trpc: {
    ...
    home: {
      ...
      pinItem: { useMutation: vi.fn((opts: any) => ({ mutate: (...args: any[]) => { mockMutate(...args); opts?.onSuccess?.(); }, isPending: false })) },
      unpinItem: { useMutation: vi.fn((opts: any) => ({ mutate: (...args: any[]) => { mockMutate(...args); opts?.onSuccess?.(); }, isPending: false })) },
    },
  },
}));
...
describe('useEntityPin', () => {
  ...
  it('returns isPinned true when entity is pinned', () => {
    const result = useEntityPin({ entityType: 'lead' as any, entityId: 'l1', title: 'Lead 1', url: '/leads/l1' });
    expect(result.isPinned).toBe(true);
  });

  it('returns isPinned false when entity is not pinned', () => {
    const result = useEntityPin({ entityType: 'contact' as any, entityId: 'c1', title: 'Contact 1', url: '/contacts/c1' });
  ...
  it('pin calls mutation when not pinned', () => {
    const result = useEntityPin({ entityType: 'contact' as any, entityId: 'c1', title: 'Contact', url: '/c/c1' });
    result.pin();
    expect(mockMutate).toHaveBeenCalled();
  });

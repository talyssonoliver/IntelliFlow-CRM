...
function ResetPasswordPage({ params }: PageParams) {
  ...
  const [tokenEmail, _setTokenEmail] = useState<string | null>(null);
  const [tokenExpiresAt, _setTokenExpiresAt] = useState<Date | null>(null);
  const [token, setToken] = useState<string>('');

  // Unwrap params using React.use()
  const resolvedParams = use(params);

  // Legacy route — redirect to /forgot-password
  // This component is kept for backward compatibility but the [token] page.tsx now redirects.
  useEffect(() => {
    ...
    // With Supabase flow, tokens are validated server-side via /reset-password/callback.
    // This legacy route shows invalid state and directs users to request a new link.
    setState('invalid');

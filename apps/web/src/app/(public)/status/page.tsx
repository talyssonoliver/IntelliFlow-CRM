...
const incidents = [
  {
    ...
    updates: [
      {
        time: '2025-12-28T14:30:00Z',
        message: 'Investigating increased response times on API endpoints.',
      },
      {
        time: '2025-12-28T14:45:00Z',
        message: 'Identified root cause as database connection pool saturation.',
      },
      { time: '2025-12-28T15:00:00Z', message: 'Deployed fix to increase connection pool size.' },
      {
        time: '2025-12-28T15:15:00Z',
        message: 'Response times back to normal. Monitoring continues.',
      },
    ],
  },
  {
...
function getOverallStatus(serviceList: Service[]) {
  ...
  const hasDegraded = serviceList.some(
    ...
    <main id="main-content" className="bg-slate-50 dark:bg-slate-900 min-h-screen">
      ...
      >
        <div className="container px-4 lg:px-6 mx-auto max-w-4xl text-center">
          ...
          <p className="text-white/90">Last updated: {formatDate(new Date().toISOString())}</p>
        </div>
      </section>
                  ...
                  <div className="border-l-2 border-slate-200 dark:border-slate-700 pl-4 space-y-3">
                    {incident.updates.map((update, idx) => (
                      <div key={idx} className="text-sm">
                        ...
                        <p className="text-slate-700 dark:text-slate-300">{update.message}</p>
                      </div>
                    ))}
                  </div>
                </Card>

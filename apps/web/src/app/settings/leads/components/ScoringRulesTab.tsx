...
export function ScoringRulesTab({ rules, onRulesChange }: Readonly<ScoringRulesTabProps>) {
  const handlePointsChange = useCallback(
    (activityType: string, value: string) => {
      ...
      onRulesChange(rules.map((r) => (r.activityType === activityType ? { ...r, points } : r)));
    },
    [rules, onRulesChange]
  );
  ...
  return (
    <div className="space-y-4">
      {rules.map((rule) => (
        <div key={rule.activityType} className="flex items-center justify-between gap-4">
          <label htmlFor={`scoring-${rule.activityType}`} className="text-sm font-medium flex-1">
          ...
          <div className="flex items-center gap-2">
            <Input
              ...
              step={1}
              value={rule.points}
              onChange={(e) => handlePointsChange(rule.activityType, e.target.value)}

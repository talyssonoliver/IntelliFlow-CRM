...
export function AutomationRuleBuilder() {
  ...
  const { data: rulesData, isLoading } = (trpc.routing.list as any).useQuery({ limit: 100 }) as { data: any; isLoading: boolean };

  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- tRPC deep type instantiation workaround
  const createMutation = (trpc.routing.create as any).useMutation({
    onSuccess: () => { utils.routing.list.invalidate(); setDialogOpen(false); toast({ title: 'Rule created' }); },
    onError: (err: { message: string }) => toast({ title: err.message, variant: 'destructive' as const }),
  });
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const updateMutation = (trpc.routing.update as any).useMutation({
    onSuccess: () => { utils.routing.list.invalidate(); setDialogOpen(false); setEditingId(null); toast({ title: 'Rule updated' }); },
    onError: (err: { message: string }) => toast({ title: err.message, variant: 'destructive' as const }),
  });
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const deleteMutation = (trpc.routing.delete as any).useMutation({
    onSuccess: () => { utils.routing.list.invalidate(); toast({ title: 'Rule deleted' }); },
    onError: (err: { message: string }) => toast({ title: err.message, variant: 'destructive' as const }),
  });
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  if (!rules.length) {
    return <ConfigEmptyState title="No Automation Rules" description="Create rules to automatically route, assign, and escalate tickets." actionLabel="Create Rule" onAction={openCreate} />;
  }
              ...
              <TableRow key={rule.id}>
                ...
                <TableCell>
                  <div className="flex gap-1">
                    ...
                    <Button variant="ghost" size="icon" onClick={() => deleteMutation.mutate({ id: rule.id })} aria-label={`Delete ${rule.name}`}>
        ...
        <DialogContent className="max-w-2xl" role="dialog">
          ...
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              ...
              <Input id="rule-name" value={formData.name} onChange={(e) => setFormData((f) => ({ ...f, name: e.target.value }))} placeholder="e.g. High Priority Billing" />
            </div>
            <div className="grid gap-2">
              <Label htmlFor="rule-desc">Description</Label>
              <Textarea id="rule-desc" value={formData.description} onChange={(e) => setFormData((f) => ({ ...f, description: e.target.value }))} />
            </div>
            <div className="grid gap-2">
              <Label htmlFor="rule-priority">Priority</Label>
              <Input id="rule-priority" type="number" min={0} value={formData.priority} onChange={(e) => setFormData((f) => ({ ...f, priority: parseInt(e.target.value) || 0 }))} />
            </div>
            ...
            <div>
              ...
              {formData.conditions.map((condition, idx) => (
                <div key={idx} className="mb-2 grid grid-cols-3 gap-2">
                  ...
                  <Input value={condition.value} onChange={(e) => updateCondition(idx, 'value', e.target.value)} placeholder="Value" />
                </div>
              ))}
              <Button variant="outline" size="sm" onClick={() => setFormData((f) => ({ ...f, conditions: [...f.conditions, { field: 'category', operator: 'equals', value: '' }] }))}>
                Add Condition
              </Button>
            </div>
            ...
            <div>
              ...
              {formData.actions.map((action, idx) => (
                <div key={idx} className="mb-2 grid grid-cols-2 gap-2">
                  ...
                  <Input value={action.target} onChange={(e) => updateAction(idx, 'target', e.target.value)} placeholder="Target" />
                </div>
              ))}
              <Button variant="outline" size="sm" onClick={() => setFormData((f) => ({ ...f, actions: [...f.actions, { type: 'assign_to_skill', target: '' }] }))}>
                Add Action
              </Button>
            </div>
          </div>

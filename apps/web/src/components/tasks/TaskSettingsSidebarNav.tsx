...
import { TASK_SETTINGS_ITEMS } from '@/components/sidebar/configs/tasks';

interface TaskSettingsSidebarNavProps {
  isExpanded: boolean;
}

/**
 * Renders the settings items inline in the sidebar when on a task settings page.
 * Task Views & Priority sections are rendered natively by the sidebar via `sections`.
 */
export function TaskSettingsSidebarNav({
  isExpanded: sidebarExpanded,
}: Readonly<TaskSettingsSidebarNavProps>) {
    ...
    <div className="flex flex-col gap-1">
      <div>
        ...
        >
          ...
          {sidebarExpanded && <span className="font-medium truncate">Task Settings</span>}
        </div>

        {sidebarExpanded ? (
          <div className="ml-3 mt-1 mb-1 flex flex-col gap-0.5">
            {TASK_SETTINGS_ITEMS.map((item) => {
      ...
      {/* ── Divider between settings and Task Views ── */}

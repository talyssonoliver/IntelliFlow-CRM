...
import { NOTIFICATION_SETTINGS_ITEMS } from '@/components/sidebar/configs/notifications';

interface NotificationSettingsSidebarNavProps {
  isExpanded: boolean;
}

/**
 * Renders the settings items inline in the sidebar when on a notification settings page.
 * Notification Center section is rendered natively by the sidebar via `sections`.
 */
export function NotificationSettingsSidebarNav({
  isExpanded: sidebarExpanded,
}: Readonly<NotificationSettingsSidebarNavProps>) {
      ...
      <div>
        ...
        {sidebarExpanded ? (
          <div className="ml-3 mt-1 mb-1 flex flex-col gap-0.5">
            {NOTIFICATION_SETTINGS_ITEMS.map((item) => {

...
interface DeadlineTrackerProps {
  tasks: CaseTaskItem[];
  onAddTask: (task: { title: string; description?: string; dueDate?: Date; assignee?: string }) => void;
  onCompleteTask: (taskId: string) => void;
  onRemoveTask: (taskId: string) => void;
  disabled?: boolean;
}

export function DeadlineTracker({ tasks, onAddTask, onCompleteTask, onRemoveTask, disabled }: DeadlineTrackerProps) {
            ...
            return (
              <li key={task.id} className="flex items-center gap-3 py-2 px-3 rounded-md border">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {task.dueDate && (
                    <span className={cn('text-xs ml-2', task.isOverdue ? 'text-red-600' : 'text-muted-foreground')}>
        ...
        showForm ? (
          <form onSubmit={handleSubmit} className="space-y-2 p-3 border rounded-md">
            ...
            <div className="flex gap-2">
              ...
              <button type="button" onClick={() => setShowForm(false)} className="px-3 py-1 text-sm border rounded-md">
                Cancel
              </button>
            </div>
          </form>

...
import { CONTACT_SETTINGS_ITEMS } from '@/components/sidebar/configs/contacts';

interface ContactSettingsSidebarNavProps {
  isExpanded: boolean;
}

/**
 * Renders the settings items inline in the sidebar when on a contact settings page.
 * Contact Views & Segments sections are rendered natively by the sidebar via `sections`.
 */
export function ContactSettingsSidebarNav({
  isExpanded: sidebarExpanded,
}: Readonly<ContactSettingsSidebarNavProps>) {
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <div>
        ...
        >
          <div className="w-7 h-7 rounded-md flex items-center justify-center flex-shrink-0 bg-blue-50 dark:bg-blue-950">
            <span
              className="material-symbols-outlined text-lg text-blue-600 dark:text-blue-400"
              aria-hidden="true"
            >
              settings
            </span>
          </div>
          {sidebarExpanded && <span className="font-medium truncate">Contact Settings</span>}
        </div>

        {sidebarExpanded ? (
          <div className="ml-3 mt-1 mb-1 flex flex-col gap-0.5">
            {CONTACT_SETTINGS_ITEMS.map((item) => {
      ...
      {/* ── Divider between settings and Contact Views ── */}

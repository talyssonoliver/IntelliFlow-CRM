...
export function RelationshipGraph({
  ...
  const hasRelationships = contact.account || linkedLead || relatedContacts.length > 0 || opportunityCount > 0 || taskCount > 0;
        ...
        <ul className="space-y-3">
          ...
          {contact.account && (
            <li className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800 rounded-lg">
              <span className="material-symbols-outlined text-primary" aria-hidden="true">domain</span>
          ...
          {!contact.account && (
            <li className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800 rounded-lg">
              <span className="material-symbols-outlined text-slate-400" aria-hidden="true">domain</span>
              <div className="flex-1">
                <p className="text-xs text-slate-500 uppercase font-semibold">Account</p>
          ...
          {linkedLead && (
            <li className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800 rounded-lg">
              <span className="material-symbols-outlined text-emerald-600" aria-hidden="true">person_search</span>
              <div className="flex-1">
                <p className="text-xs text-slate-500 uppercase font-semibold">Converted from Lead</p>
                <Link
          ...
          {relatedContacts.length > 0 && (
            <li className="p-3 bg-slate-50 dark:bg-slate-800 rounded-lg">
              <div className="flex items-center gap-2 mb-2">
                <span className="material-symbols-outlined text-indigo-600" aria-hidden="true">group</span>
                <p className="text-xs text-slate-500 uppercase font-semibold">Related Contacts ({relatedContacts.length})</p>
              </div>
          ...
          {taskCount > 0 && (
            <li className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800 rounded-lg">
              <span className="material-symbols-outlined text-sky-600" aria-hidden="true">task_alt</span>
              <div className="flex-1">

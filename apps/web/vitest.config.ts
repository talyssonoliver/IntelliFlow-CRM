...
export default defineConfig({
  ...
  test: {
    ...
    // component renders / module-graph imports 2-3x slower — so 30s spuriously times
    // out (flaky, passes in isolation/sharded CI). Raise the budget when instrumented
    // so coverage measures real failures, not instrumentation overhead. (IFC-242)

...
async function parseRiskRegister(): Promise<{ risks: Risk[]; lastUpdated: string | null }> {
  try {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const impact = Number.parseInt(getValue('impact')) || 3;
      const likelihood = Number.parseInt(getValue('likelihood')) || 3;
      const score = Number.parseInt(getValue('score')) || impact * likelihood;
...
async function handleAddRisk(body: any): Promise<NextResponse> {
...
function applyRiskUpdates(currentRisk: Risk, updates: any, auditEntry: RiskAuditEntry): Risk {
...
async function handleEditRisk(body: any): Promise<NextResponse> {
  const parseResult = EditRiskSchema.safeParse({ riskId: body.riskId, updates: body.updates });
  if (!parseResult.success) {
    return NextResponse.json(
      { status: 'error', message: 'Validation failed', errors: parseResult.error.issues },
      { status: 400 }
  ...
  const updatedRisk: Risk = applyRiskUpdates(currentRisk, updates, auditEntry);
  ...
  return NextResponse.json({ status: 'ok', message: `Risk ${riskId} updated`, riskId });
}

const ACTION_HANDLERS: Record<string, (body: any) => Promise<NextResponse>> = {
  add: handleAddRisk,
  edit: handleEditRisk,
};

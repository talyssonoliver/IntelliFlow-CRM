...
function getDirectoryIcon(dir: Readonly<DirectoryType>) {
...
function getDirectoryColor(dir: Readonly<DirectoryType>): string {
...
// =============================================================================
// PROMPT BUILDER (extracted from component to reduce cognitive complexity)
// =============================================================================
...
function buildFileAuditPrompt(
  ...
  linkedFiles: FileEntry[]
): string {
  const expId = buildExpTaskId(folderPath);

  return `# File Audit Prompt for \`${folderPath}/\`
...
${expId},Cleanup,Audit and document ${folderPath} orphan files,Dev Team (STOA-Quality),None,FILE:audit-matrix.yml,Files reviewed and documented; unused files removed or archived; remaining files linked to tasks,100% files accounted for,AUDIT:manual-review,Continuous,"EVIDENCE:artifacts/attestations/${expId}/context_ack.json",Backlog
\`\`\`
...
${buildLinkedFilesList(linkedFiles)}
...
function accumulateFolderEntry(
  file: FileEntry,
  folderPath: string,
  folderName: string,
  parts: string[],
...
function getFolderContents(
  ...
  filesAtPath.forEach((file) => {
    ...
    if (parts.length === 1) {
      ...
      accumulateFolderEntry(file, folderPath, folderName, parts, folderMap);
    }
  });

  const folders = Array.from(folderMap.values()).sort((a, b) => {
    if (b.orphanCount !== a.orphanCount) return b.orphanCount - a.orphanCount;
      ...
      {file.linkedTasks.length > 0 && (
        <div className="flex items-center gap-1 ml-4">
          {file.linkedTasks.slice(0, 2).map((taskId) => (
            <LinkedTaskButton key={taskId} taskId={taskId} onTaskClick={onTaskClick} />
          ))}
          {file.linkedTasks.length > 2 && (
            <span className="text-xs text-gray-500">+{file.linkedTasks.length - 2}</span>
          )}
        </div>
      )}
    </div>
...
function getPrefixColor(prefix: string): string {
  if (prefix === 'ARTIFACT') return 'bg-blue-100 text-blue-800';
  if (prefix === 'EVIDENCE') return 'bg-purple-100 text-purple-800';
        ...
        <div className="bg-white rounded-lg shadow overflow-hidden">
          ...
          <div className="divide-y divide-gray-100 max-h-[400px] overflow-y-auto">
            {directFiles.map((file) => (
              <FileListItem key={file.path} file={file} onTaskClick={_onTaskClick} />
            ))}
          </div>
        </div>
      )}
...
export default function ArtifactsView({ onTaskClick }: Readonly<ArtifactsViewProps>) {
  ...
  const generateFileAuditPrompt = async (folderPath: string) => {
    ...
    try {
      ...
      const prompt = buildFileAuditPrompt(folderPath, folderFiles, orphanFiles, linkedFiles);
  ...
  const downloadPrompt = () => {
    ...
    a.href = url;
    a.download = `FileAudit_${generatedPrompt.dir.replaceAll('/', '_')}_prompt.md`;
    ...
    <div className="space-y-6">
      ...
      {activeTab === 'files' && (
        <FilesTabContent
          ...
          generateFileAuditPrompt={generateFileAuditPrompt}
                ...
                {codeAnalysis.knip.data.files.length > 0 && (
                  <div className="p-4">
                    ...
                    <div className="max-h-48 overflow-y-auto">
                      {codeAnalysis.knip.data.files.slice(0, 50).map((file) => (
                        <div key={file} className="text-sm font-mono text-gray-600 py-1">
                          {file}
                        </div>
                      ))}
              ...
              <div className="divide-y">
                ...
                {codeAnalysis.depcheck.data.devDependencies.length > 0 && (
                  <div className="p-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {codeAnalysis.depcheck.data.devDependencies.map((dep) => (
                        <span
                          key={dep}
                          className="px-2 py-1 bg-orange-100 text-orange-700 rounded text-sm"
                        >

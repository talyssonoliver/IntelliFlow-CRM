...
// ============================================================================
// Active-sprint fallback
// ============================================================================

/**
 * Derives `active_sprint` from the `task_details` map when the registry was
 * generated by an older generator that didn't populate the field directly.
 * Returns the lowest sprint that still has non-DONE tasks, or the highest
 * sprint seen if everything is done.
 */
function deriveActiveSprintFromTaskDetails(registry: TaskRegistry): void {
  if (registry.active_sprint) return;
...
export function generateContextSnapshot(metricsDir: string, repoDir: string): SnapshotResult {
  ...
  deriveActiveSprintFromTaskDetails(registry);

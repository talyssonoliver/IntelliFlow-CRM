...
const mockPrisma = {
  auditLog: {
    create: vi.fn().mockResolvedValue({ id: 'aid' }),
...
vi.mock('../audit-logger', () => {
  class AL {
    ...
    async logAction(a: string, et: string, ei: string, ti: string, o?: any) {
      await this.p.auditLog.create({ data: { a, et, ei, ti, ...o } });
      return 'a-1';
    }
    async logPermissionDenied(et: string, ei: string, perm: string, ti: string, o?: any) {
      await this.p.auditLog.create({ data: { ...o } });
      return 'pd-1';
    }
    async logLoginSuccess(ti: string, o?: any) {
      await this.p.auditLog.create({ data: { ...o } });
      await this.p.securityEvent.create({ data: { eventType: 'LOGIN_SUCCESS' } });
    }
    async logLoginFailure(ti: string, o?: any) {
      await this.p.auditLog.create({ data: { ...o } });
      await this.p.securityEvent.create({
        data: { eventType: 'LOGIN_FAILURE', severity: 'MEDIUM' },
      });
    }
    async logBulkOperation(a: string, et: string, ids: string[], ti: string, o?: any) {
      await this.p.auditLog.create({ data: { ...o } });
      return 'b-1';
    }
  }
...
vi.mock('../rbac', () => {
  const P: Record<string, Record<string, string[]>> = {
    ADMIN: {
      lead: ['read', 'write', 'delete', 'admin', 'manage', 'export'],
      contact: ['read', 'write', 'delete', 'admin', 'manage', 'export'],
      account: ['read', 'write', 'delete', 'admin', 'manage'],
      opportunity: ['read', 'write', 'delete', 'admin', 'manage'],
      task: ['read', 'write', 'delete', 'admin', 'manage'],
      appointment: ['read', 'write', 'delete', 'admin', 'manage'],
      user: ['read', 'write', 'delete', 'admin'],
    },
    MANAGER: {
      lead: ['read', 'write', 'delete', 'manage', 'export'],
      contact: ['read', 'write', 'delete', 'manage', 'export'],
      account: ['read', 'write', 'delete', 'manage'],
      opportunity: ['read', 'write', 'delete', 'manage'],
      task: ['read', 'write', 'delete', 'manage'],
      appointment: ['read', 'write', 'delete', 'manage'],
      user: ['read', 'write'],
    },
    SALES_REP: {
      lead: ['read', 'write', 'delete'],
      contact: ['read', 'write', 'delete'],
      account: ['read', 'write'],
      opportunity: ['read', 'write', 'delete'],
  ...
  class R {
    ...
    async can(o: any) {
      ...
      if (
        o.action !== 'read' &&
        o.resourceOwnerId &&
        o.resourceOwnerId !== o.userId &&
        this.getRoleLevel(o.userRole) < 4
      )
    ...
    async getPermissions(u: string, r: string) {
      const k = u + ':' + r;
      if (this.c.has(k)) return this.c.get(k)!;
      const ps: string[] = [];
      for (const [res, acts] of Object.entries(P[r] || {})) {
        for (const a of acts) ps.push(res + ':' + a);
      }
      this.c.set(k, ps);
      return ps;
    }
    clearCache(u: string) {
      for (const k of this.c.keys()) {
        if (k.startsWith(u)) this.c.delete(k);
      }
    }
    getRoleLevel(r: string) {
      return ({ VIEWER: 1, USER: 2, SALES_REP: 3, MANAGER: 4, ADMIN: 5 } as any)[r] || 0;
    }
    isManager(r: string) {
      return this.getRoleLevel(r) >= 4;
    }
    isAdmin(r: string) {
      return r === 'ADMIN';
    }
    ...
    private h(role: string, res: string, act: string) {
      return (P[role]?.[res] || []).includes(act);
    }
  }
  return {
    RBACService: R,
...
describe('audit-coverage-test.ts source coverage', () => {
  ...
  it('should cover CRUD operations', async () => {
    ...
    await l.logAction('UPDATE', 'lead', 'l1', 't1', {
      actorId: 'u1',
      beforeState: { s: 'A' },
      afterState: { s: 'B' },
    });
  ...
  it('should cover RBAC roles and ABAC', async () => {
    ...
    expect(r.evaluateConditions([{ field: 's', operator: 'eq', value: 'A' }], { s: 'A' })).toBe(
      true
    );
    expect(r.evaluateConditions([{ field: 's', operator: 'neq', value: 'X' }], { s: 'A' })).toBe(
      true
    );
    expect(r.evaluateConditions([{ field: 'r', operator: 'in', value: ['A'] }], { r: 'A' })).toBe(
      true
    );

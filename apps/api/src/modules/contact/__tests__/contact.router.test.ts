...
describe('Contact Router', () => {
  ...
  describe('list', () => {
    it('should list contacts with pagination', async () => {
      const contacts = [mockContact, { ...mockContact, id: 'contact-2', email: 'contact2@example.com' }];
  ...
  describe('update', () => {
    ...
    it('should throw NOT_FOUND when updating non-existent contact', async () => {
      ...
      await expect(
        caller.update({ id: TEST_UUIDS.nonExistent, title: 'Test' })
      ).rejects.toThrow(
        expect.objectContaining({
          code: 'NOT_FOUND',
        })
  ...
  describe('unlinkFromAccount', () => {
    ...
    it('should throw NOT_FOUND for non-existent contact', async () => {
      ...
      await expect(
        caller.unlinkFromAccount({ contactId: TEST_UUIDS.nonExistent })
      ).rejects.toThrow(
        expect.objectContaining({
          code: 'NOT_FOUND',
        })

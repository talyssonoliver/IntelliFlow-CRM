...
describe('Workflow CRUD Procedures', () => {
  ...
  describe('workflow.create', () => {
    it('creates a workflow with tenantId from context', async () => {
      (prismaMock.workflowDefinition.create as ReturnType<typeof vi.fn>).mockResolvedValue(baseWorkflow);
    ...
    it('throws CONFLICT when workflow name already exists in same tenant', async () => {
      ...
      (prismaMock.workflowDefinition.create as ReturnType<typeof vi.fn>).mockRejectedValue(prismaError);
    ...
    it('allows same name reuse after soft-delete (AC-007)', async () => {
      ...
      (prismaMock.workflowDefinition.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue(baseWorkflow);
      const softDeleted = { ...baseWorkflow, deletedAt: new Date('2026-04-02T00:00:00Z') };
      (prismaMock.workflowDefinition.update as ReturnType<typeof vi.fn>).mockResolvedValue(softDeleted);
  ...
  describe('workflow.update', () => {
    ...
    it('throws NOT_FOUND when workflow belongs to different tenant (cross-tenant isolation)', async () => {
      ...
      await expect(
        caller.update({ id: workflowId1, name: 'Hijacked' }),
  ...
  describe('workflow.setActive', () => {
    it('toggles isActive flag', async () => {
      ...
      (prismaMock.workflowDefinition.update as ReturnType<typeof vi.fn>).mockResolvedValue(deactivated);
    ...
    it('throws NOT_FOUND for deleted workflow', async () => {
      ...
      await expect(caller.setActive({ id: TEST_UUIDS.nonExistent, isActive: true })).rejects.toMatchObject(
        expect.objectContaining({ code: 'NOT_FOUND' }),
      );
    });
  });
  ...
  describe('workflow.list', () => {
    it('returns paginated results with cursor', async () => {
      const workflows = [baseWorkflow, { ...baseWorkflow, id: workflowId2, name: 'Ticket Routing' }];
      (prismaMock.workflowDefinition.findMany as ReturnType<typeof vi.fn>).mockResolvedValue(workflows);
    ...
    it('excludes records where deletedAt IS NOT NULL', async () => {
      ...
      (prismaMock.workflowDefinition.findMany as ReturnType<typeof vi.fn>).mockResolvedValue(activeOnly);
    ...
    it('tenant A cannot see tenant B workflows (tenant isolation)', async () => {
      (prismaMock.workflowDefinition.findMany as ReturnType<typeof vi.fn>).mockResolvedValue([baseWorkflow]);

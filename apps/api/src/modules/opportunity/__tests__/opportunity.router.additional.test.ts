...
describe("opportunityRouter additional coverage", () => {
  ...
  describe("create - generic BAD_REQUEST", () => {
    it("should throw BAD_REQUEST for generic service error", async () => {
      ...
      await expect(caller.create({
        ...
        stage: "PROSPECTING",
        value: { amount: 10000, currency: "GBP" }, probability: 50,
  ...
  describe("list - probability filters", () => {
    it("should apply minProbability and maxProbability filters", async () => {
      ...
      const result = await caller.list({
        minProbability: 25, maxProbability: 75,
      });
  ...
  describe("list - date filters", () => {
    it("should apply dateFrom and dateTo filters", async () => {
      ...
      const result = await caller.list({
        dateFrom: new Date("2025-01-01"), dateTo: new Date("2025-12-31"),
      });
  ...
  describe("moveStage - CLOSED_LOST without reason", () => {
    it("should pass empty reason when not provided", async () => {
      ...
      const result = await caller.moveStage({
        id: OPP_ID, targetStage: "CLOSED_LOST",
      });
    ...
    it("should pass reason when provided", async () => {
      ...
      await caller.moveStage({
        id: OPP_ID, targetStage: "CLOSED_LOST", reason: "Budget cut",
      });

      expect(mockServices.opportunity.markAsLost).toHaveBeenCalledWith(
        OPP_ID, "Budget cut", USER_ID
      );
    });
  });
  ...
  describe("forecast - with data", () => {
    it("should calculate weighted value and win rate from real data", async () => {
      const activeOpps = [
        { id: "opp-1", name: "Deal A", stage: "PROPOSAL", value: 10000, probability: 60, expectedCloseDate: new Date(), owner: { id: "u1", name: "Alice", email: "a@e.com" }, account: { id: "a1", name: "Corp" } },
        { id: "opp-2", name: "Deal B", stage: "NEGOTIATION", value: 20000, probability: 80, expectedCloseDate: new Date(), owner: { id: "u2", name: "Bob", email: "b@e.com" }, account: null },
      ];
      const closedDeals = [
        { id: "c-1", value: 15000, stage: "CLOSED_WON", closedAt: new Date(), createdAt: new Date(Date.now() - 30 * 86400000) },
        { id: "c-2", value: 8000, stage: "CLOSED_LOST", closedAt: new Date(), createdAt: new Date(Date.now() - 20 * 86400000) },
      ];
  ...
  describe("getOpportunityService - missing service", () => {
    it("should throw INTERNAL_SERVER_ERROR when service unavailable", async () => {
      ...
      await expect(caller.getById({ id: OPP_ID })).rejects.toThrow("Opportunity service not available");
    });
  });
});

...
export const activityFeedRouter = createTRPCRouter({
  ...
  getStats: tenantProcedure
    .input(activityFeedStatsQuerySchema)
   ...
   */
  toggleReaction: tenantProcedure
   ...
   */
  getReactions: tenantProcedure
    .input(getReactionsSchema)
    .query(async ({ ctx, input }) => {
      const tenantId = ctx.tenant.tenantId;
  ...
  addComment: tenantProcedure
    .input(addActivityCommentSchema)
    .mutation(async ({ ctx, input }) => {
      const tenantId = ctx.tenant.tenantId;
      const userId = ctx.tenant.userId;
      const userName = ctx.user?.email ?? 'Unknown';
   ...
   */
  getComments: tenantProcedure
    .input(getActivityCommentsSchema)
    .query(async ({ ctx, input }) => {
      const tenantId = ctx.tenant.tenantId;

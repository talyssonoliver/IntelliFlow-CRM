...
export const calendarWebhooksRouter = createTRPCRouter({
  getSyncStatus: protectedProcedure.input(providerInput).query(async ({ input }) => {
    return {
  ...
  triggerSync: protectedProcedure.input(providerInput).mutation(async ({ input }) => {
    return {
      provider: input.provider,
      triggered: true,
  ...
  listRegistrations: protectedProcedure.query(async () => {
    return {

/**
 * OpenTelemetry SDK Configuration — AI Worker
 *
 * Mirrors apps/api/src/tracing/otel.ts. Must be imported before any other
 * module so that auto-instrumentation patches are applied before LangChain,
 * BullMQ, or express load.
 *
 * Environment Variables:
 * - OTEL_ENABLED: Enable/disable tracing (default: true)
 * - OTEL_SERVICE_NAME: Service name (default: intelliflow-ai-worker)
...
function getOtelConfig(): OtelConfig {
  ...
  return {
    ...
    serviceName: envString(process.env.OTEL_SERVICE_NAME, 'intelliflow-ai-worker'),
    serviceVersion: envString(process.env.npm_package_version, '0.1.0'),
    environment,
    otlpEndpoint: envString(process.env.OTEL_EXPORTER_OTLP_ENDPOINT, 'http://localhost:4317'),
...
export function initializeOpenTelemetry(): NodeSDK | null {
  ...
  // In development without a custom OTLP endpoint, fall back to console so
  // ECONNREFUSED noise is avoided when the collector is not running locally.
...
// Start tracing immediately on import — OTel must be active before any
// instrumented library (LangChain, BullMQ, express) is loaded.
startTracing();

...
describe('ChurnRiskChain additional', () => {
  ...
  describe('fallback heuristics', () => {
    ...
    it('combined factors clamp to 1.0', async () => {
      ...
      const r = await chain.predictChurnRisk({
        ...baseInput,
        daysSinceLastLogin: 90,
        npsScore: 2,
        supportTickets30d: 15,
        usageTrendSlope: -1.0,
      });
  ...
  describe('input schema validation', () => {
    ...
    it('rejects invalid uuid', () => {
      expect(() =>
        churnRiskInputSchema.parse({ entityType: 'lead', entityId: 'not-a-uuid' })
      ).toThrow();
    });

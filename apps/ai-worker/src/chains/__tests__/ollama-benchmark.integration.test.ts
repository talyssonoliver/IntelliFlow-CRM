...
describe.skipIf(!runOllamaBenchmarks)('GATE: Real Ollama Accuracy Benchmark', () => {
  ...
  beforeAll(async () => {
    ...
    if (!ollamaAvailable) {
      console.warn('[IFC-174] Ollama not available — skipping integration tests');
      return;
    }
    ...
    console.log(`[IFC-174] Scored ${allResults.length} leads across ${iterations} iterations`);
  ...
  it('scores high-quality leads with score > 50', () => {
    ...
    const avgScore = highResults.reduce((sum, r) => sum + r.result.score, 0) / highResults.length;
  ...
  it('scores low-quality leads with score < high-quality average', () => {
    ...
    const highAvg = highResults.reduce((sum, r) => sum + r.result.score, 0) / highResults.length;
    const lowAvg = lowResults.reduce((sum, r) => sum + r.result.score, 0) / lowResults.length;
  ...
  it('score differentiation >= 20 points', () => {
    ...
    console.log(`[IFC-174] Score differentiation: ${differentiation.toFixed(1)}`);
  });
  ...
  it('zero errors — all scoring calls return successfully', () => {
    ...
    const errors = allResults.filter((r) => r.result.modelVersion === 'error:v1');
  ...
  it('tier ordering — high avg > medium avg > low avg', () => {
    ...
    const highAvg = highResults.reduce((sum, r) => sum + r.result.score, 0) / highResults.length;
    const mediumAvg =
      mediumResults.reduce((sum, r) => sum + r.result.score, 0) / mediumResults.length;
    const lowAvg = lowResults.reduce((sum, r) => sum + r.result.score, 0) / lowResults.length;
